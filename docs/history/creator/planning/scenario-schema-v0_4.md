...
**v0.4 · Draft for review · The contract between Creator (produces) and Player (consumes) · Last updated: 2026-06-25**

> **Update (v0.4, 2026-06-25) — first JSON change since 0.3.0.** Closes the F library sub-deferrals and adds MVP-scoped per-kind node `props`. Specifically: `playerCountScaling.dormantKingdoms`, room `improveOnce`/`enterRequirement`, quest `requirements` (via the closed `condition` vocabulary), foe `strike`/`movement`, and `battleDefs.*.cards` are now specified; the closed **`tradeAsset`** union is added (rules-engine decision #7: `warriors|spirit|item|companion`, virtues/corruptions structurally untradeable) and wired into `action.trade`; and per-kind `props` are now specified for the MVP node set (`action.trade`/`banner`, `dungeon.subflow`, `event.companion`, `winloss.mainGoal`/`winCondition`/`lossCondition`, `cond.check`/`branch`, `trigger.schedule`/`onState`, `media.narration`/`showImage`) on top of the v0.3 spine (`effect.apply`, `tower.op`). **Battle/foe figures are author-editable DEFAULTS, never fixed constraints** (per Official App Analysis v0.3 §6: observed, unverified). The conformance suite grows from 12 → **38 cases (all green, ajv strict / draft 2020-12)**. Reconstruction note: this schema was rebuilt from the v0.3 §11 embed, which had drifted from the canonical file; five strict-mode/spine defects were corrected to restore the green v0.3 baseline before adding v0.4 — see §14.
...
The `kind` enum is **closed** and mirrors the block catalog §3–§11 one-to-one. v0.3 specifies the spine `props` (`effect.apply`, `tower.op`); **v0.4 adds per-kind `props` for the MVP node set** — `action.trade`/`banner`, `dungeon.subflow`, `event.companion`, `winloss.mainGoal`/`winCondition`/`lossCondition`, `cond.check`/`branch`, `trigger.schedule`/`onState`, `media.narration`/`showImage` — via the `node.allOf` discriminator. Remaining `kind`s stay enumerated (so the vocabulary is closed) and keep open `props` pending later v0.4 passes (§13/§14). The full list, by category:
...
```json
{
  ...
  "$defs": {
    ...
    "tradeAsset": {
      "type": "object",
      "oneOf": [
        { "additionalProperties": false, "required": ["asset", "amount"],
          "properties": { "asset": { "enum": ["warriors", "spirit"] }, "amount": { "type": "integer", "minimum": 1 } } },
        { "additionalProperties": false, "required": ["asset", "itemRef"],
          "properties": { "asset": { "const": "item" }, "itemRef": { "type": "string" } } },
        { "additionalProperties": false, "required": ["asset", "companionId"],
          "properties": { "asset": { "const": "companion" }, "companionId": { "$ref": "#/$defs/id" } } }
      ],
      "$comment": "v0.4 — closed TradeAsset union (rules-engine decision #7): warriors|spirit|item|companion. Virtues & corruptions are structurally untradeable (absent by construction)."
    },
            ...
            "dormantKingdoms": {
              ...
              "properties": {
                "byPlayerCount": {
                  ...
                  "additionalProperties": false,
                  "properties": {
                    "1": { "type": "array", "items": { "$ref": "#/$defs/kingdom" }, "uniqueItems": true },
                    "2": { "type": "array", "items": { "$ref": "#/$defs/kingdom" }, "uniqueItems": true },
                    "3": { "type": "array", "items": { "$ref": "#/$defs/kingdom" }, "uniqueItems": true },
                    "4": { "type": "array", "items": { "$ref": "#/$defs/kingdom" }, "maxItems": 0 }
                  },
                  "$comment": "dormant kingdoms per player count; 4P has none. L3: a kingdom is dormant in at most a contiguous setup"
                },
                "skullRedirect": { "enum": ["nearestActive", "dropperChoice"], "default": "nearestActive",
                  "$comment": "where a skull bound for a dormant kingdom is placed instead" }
              },
              "$comment": "v0.4 — catalog §13 Player-Count Scaling. Structural (not content-blocked)."
            }
          }
        },
        "monthEnd": {
          ...
          "oneOf": [
            {
              ...
              "properties": { "boardStateRef": { "$ref": "#/$defs/resourceKey" } }
            },
            {
              "additionalProperties": false,
              "required": ["boardState"],
              "properties": { "boardState": { "type": "object", "$comment": "opaque — validated by Board's schema @pins.board" } }
            }
          ]
        }
      }
    ...
    "effect": {
      "type": "object",
      "required": ["op"],
      "$comment": "Closed verb vocabulary — catalog §F. MVP-critical ops fully specified; remainder enumerated, props TBD v0.4. Ops without an allOf branch are valid by enum but props are unconstrained pending v0.4; layer-3 still applies.",
      "properties": {
        "op": {
          "enum": [
      ...
      "allOf": [
        ...
        { "if": { "properties": { "op": { "const": "corruption.remove" } } },
          "then": { "properties": { "op": true },
            "oneOf": [
              { "required": ["all"], "properties": { "all": { "const": true } } },
              { "required": ["count"], "properties": { "count": { "type": "integer", "minimum": 1 } } }
            ],
            "unevaluatedProperties": false,
            "$comment": "Sanctuary enhanced uses {all:true}; count form removes N (surfaced in MVP fixture verification)" } }
      ]
    },
    ...
    "towerOp": {
      ...
      "allOf": [
        ...
        { "if": { "properties": { "channel": { "const": "sound" } } },
          "then": { "oneOf": [
              { "required": ["clipIndex"], "properties": { "clipIndex": { "type": "integer", "minimum": 1 } } },
              { "required": ["category"], "properties": { "category": { "enum": ["Adversary", "Ally", "Battle", "Classic", "Unlisted", "Dungeon",
    ...
    "room": {
      ...
      "properties": {
        ...
        "improveOnce": {
          ...
          "properties": { "effects": { "type": "array", "minItems": 1, "items": { "$ref": "#/$defs/effect" } } },
          "$comment": "v0.4 — catalog §5. One-time room reward, applied once per dungeon instance."
        },
        "enterRequirement": {
          "type": "object", "additionalProperties": false,
          "properties": {
            "condition": { "$ref": "#/$defs/condition" },
            "spiritCost": { "type": "integer", "minimum": 1 },
            "onFail": { "type": "array", "items": { "$ref": "#/$defs/effect" } }
          },
          "$comment": "v0.4 — catalog §5. Gate on a condition and/or a spirit tax (glyph-gate-shaped); onFail effects if unmet."
        }
      }
    },
      ...
      "properties": {
        ...
        "foes": { "type": "object", "additionalProperties": {
          ...
          "properties": { "foeId": { "$ref": "#/$defs/foeId" }, "startingStatus": { "$ref": "#/$defs/foeStatus" },
            ...
            "strike": {
              "type": "object", "additionalProperties": false,
              "properties": {
                "cadence": { "enum": ["monthly", "onTowerActs", "scripted"], "$comment": "glossary: a foe strikes ~once a month" },
                "effects": { "type": "array", "items": { "$ref": "#/$defs/effect" } },
                "note": { "type": "string" }
              },
              "$comment": "v0.4 — catalog §13 Foe. Author-editable DEFAULTS (official-app-analysis v0.3 §6: observed, unverified — NOT a fixed constraint)."
            },
            "movement": {
              "type": "object", "additionalProperties": false,
              "properties": {
                "pattern": { "enum": ["toward-nearest-hero", "toward-tower", "stationary", "scripted"] },
                "spaces": { "type": "integer", "minimum": 0 },
                "note": { "type": "string" }
              },
              "$comment": "v0.4 — catalog §13 Foe. Author-editable DEFAULTS (observed, unverified per analysis v0.3 §6)."
            } } } },
        "buildingTypes": {
        ...
        "quests": { "type": "object", "additionalProperties": {
          ...
          "properties": { "id": { "$ref": "#/$defs/id" }, "name": { "type": "string" },
            ...
            "requirements": { "type": "array", "items": {
              ...
              "properties": { "label": { "type": "string" }, "condition": { "$ref": "#/$defs/condition" } },
              "$comment": "v0.4 — catalog §13 Quest. Reuses the closed condition vocabulary (compose, never invent)." } },
        ...
        "battleDefs": { "type": "object", "additionalProperties": {
          ...
          "properties": { "cards": { "type": "array", "items": {
            "type": "object", "additionalProperties": false, "required": ["advantage"],
            "properties": {
              "advantage": { "$ref": "#/$defs/advantage" },
              "strikes": { "type": "integer", "minimum": 0 },
              "critical": { "type": "boolean", "default": false },
              "onResolve": { "type": "array", "items": { "$ref": "#/$defs/effect" } },
              "note": { "type": "string" }
            },
            "$comment": "v0.4 — battle-card outcome keyed to an advantage. Author-editable DEFAULTS; deck size/keyword structure NOT enum-locked (analysis v0.3 §3/§6: observed, unverified). Cards drawn per battle = foe level (rules) is owned by the engine, not encoded here." } } } } },
    ...
    "node": {
      ...
      "properties": {
        ...
        "props": { "type": "object", "$comment": "per-kind subschema via this node's allOf. v0.3 spine: effect.apply, tower.op. v0.4 (MVP-scoped): action.trade/banner, dungeon.subflow, event.companion, winloss.mainGoal/winCondition/lossCondition, cond.check/branch, trigger.schedule/onState, media.narration/showImage. Remaining kinds keep open props pending later v0.4 passes." },
      ...
      "allOf": [
        { "$comment": "effect.apply → props carries a single effect or an effects[] (v0.3 spine)",
          "if": { "properties": { "kind": { "const": "effect.apply" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object",
            "oneOf": [
              { "additionalProperties": false, "required": ["effect"],
                "properties": { "effect": { "$ref": "#/$defs/effect" } } },
              { "additionalProperties": false, "required": ["effects"],
                "properties": { "effects": { "type": "array", "minItems": 1, "items": { "$ref": "#/$defs/effect" } } } }
            ] } } } },
        { "$comment": "tower.op → props.towerOp validates against the closed towerOp vocabulary (v0.3 spine)",
          "if": { "properties": { "kind": { "const": "tower.op" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["towerOp"],
            "properties": { "towerOp": { "$ref": "#/$defs/towerOp" } } } } } },
        { "$comment": "v0.4 — action.trade: give/receive over the closed TradeAsset union (decision #7)",
          "if": { "properties": { "kind": { "const": "action.trade" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["from", "to", "give"],
            "properties": {
              "from": { "type": "string", "$comment": "acting hero ref" },
              "to": { "type": "string", "$comment": "co-located hero ref" },
              "give": { "type": "array", "minItems": 1, "items": { "$ref": "#/$defs/tradeAsset" } },
              "receive": { "type": "array", "items": { "$ref": "#/$defs/tradeAsset" } }
            } } } } },
        { "$comment": "v0.4 — action.banner",
          "if": { "properties": { "kind": { "const": "action.banner" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["title"],
            "properties": { "title": { "type": "string" }, "body": { "type": "string" },
              "splashResource": { "$ref": "#/$defs/resourceKey" } } } } } },
        { "$comment": "v0.4 — dungeon.subflow references a library dungeon",
          "if": { "properties": { "kind": { "const": "dungeon.subflow" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["dungeonId"],
            "properties": { "dungeonId": { "$ref": "#/$defs/id" } } } } } },
        { "$comment": "v0.4 — event.companion",
          "if": { "properties": { "kind": { "const": "event.companion" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false,
            "properties": { "companionId": { "$ref": "#/$defs/id" },
              "month": { "type": "integer", "minimum": 1, "maximum": 6 } } } } } },
        { "$comment": "v0.4 — winloss.mainGoal points at the main-goal quest",
          "if": { "properties": { "kind": { "const": "winloss.mainGoal" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["questId"],
            "properties": { "questId": { "$ref": "#/$defs/id" } } } } } },
        { "$comment": "v0.4 — winloss.winCondition / lossCondition carry a condition",
          "if": { "properties": { "kind": { "enum": ["winloss.winCondition", "winloss.lossCondition"] } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["condition"],
            "properties": { "condition": { "$ref": "#/$defs/condition" } } } } } },
        { "$comment": "v0.4 — cond.check / cond.branch carry a condition",
          "if": { "properties": { "kind": { "enum": ["cond.check", "cond.branch"] } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["condition"],
            "properties": { "condition": { "$ref": "#/$defs/condition" } } } } } },
        { "$comment": "v0.4 — trigger.schedule / trigger.onState carry a trigger",
          "if": { "properties": { "kind": { "enum": ["trigger.schedule", "trigger.onState"] } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["trigger"],
            "properties": { "trigger": { "$ref": "#/$defs/trigger" } } } } } },
        { "$comment": "v0.4 — media.narration",
          "if": { "properties": { "kind": { "const": "media.narration" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["text"],
            "properties": { "text": { "type": "string" } } } } } },
        { "$comment": "v0.4 — media.showImage",
          "if": { "properties": { "kind": { "const": "media.showImage" } }, "required": ["kind"] },
          "then": { "properties": { "props": {
            "type": "object", "additionalProperties": false, "required": ["imageRef"],
            "properties": { "imageRef": { "$ref": "#/$defs/resourceKey" } } } } } }
      ]
    },
...
> **Resolved in v0.4.** The deferred "F library" items are now specified — `setup.playerCountScaling.dormantKingdoms`, room `improveOnce`/`enterRequirement`, quest `requirements`, foe `strike`/`movement`, `battleDefs.*.cards` — plus the closed `tradeAsset` union and the MVP per-kind node-prop set (§14). Still genuinely blocked: rich hero data (#2, content), Board `BoardState` shape (#3, repo), and the goal→companion mapping table (#5, content).

---

## 14. v0.4 changelog & reconstruction notes

This schema was rebuilt from the v0.3 §11 embed, which had **drifted from the canonical file** (the project's known failure mode: prose embeds lag the real artifact). The embed parsed but did not compile clean under the project's own ajv options (`{ strict: true, allErrors: true }`, draft 2020-12) and was missing branches the 12-case conformance suite proves existed. Five corrections were applied to restore a green v0.3 baseline **before** any v0.4 work, then v0.4 was layered on top.

### 14.1 Reconstruction drift-corrections (embed → canonical)

1. **`setup.board` oneOf made strict-safe.** The embed's bare `oneOf: [{required:[boardStateRef]},{required:[boardState]}]` tripped `strictRequired` (the required props were declared only in the parent). Restructured so each branch declares its own property with per-branch `additionalProperties:false` — which also correctly enforces exactly-one.
2. **Removed the non-standard `$comment-extension` key.** Unknown keyword → rejected by strict mode; its note was folded into the adjacent `$comment`.
3. **Restored the `corruption.remove` effect branch.** Absent from the embed, yet two conformance cases exercise it (Sanctuary's enhanced `{all:true}` and the `{count:N}` form). Re-added as a `oneOf` over `all|count` with `unevaluatedProperties:false`.
4. **Restored per-kind node-`props` discrimination.** The embed's `node.props` was an open `{type:object}`, but three cases require it (`effect.apply` must carry `effect`/`effects[]`; `tower.op` must validate `props.towerOp` against the closed `towerOp` vocabulary). Re-added as a `node.allOf` discriminator.
5. **Made the `towerOp.sound` branch strict-safe.** It was dormant dead-code in the embed (`towerOp` was never `$ref`'d, so ajv never compiled it); wiring it under node-props in (4) activated its strict-unsafe `oneOf` (`clipIndex`/`category`). Each branch now declares its own property.

After 14.1: **12/12 baseline cases green, strict mode clean.**

### 14.2 v0.4 additions

- **F library closures.** `dormantKingdoms` (`byPlayerCount` 1–4 with a 4-player `maxItems:0` guard + `skullRedirect` policy); room `improveOnce` (`effects[]`) and `enterRequirement` (`condition`/`spiritCost`/`onFail`); quest `requirements` (array of `{label?, condition}` reusing the closed `condition` vocabulary); foe `strike` (`cadence`/`effects`/`note`) and `movement` (`pattern`/`spaces`/`note`); `battleDefs.*.cards` (`advantage`/`strikes`/`critical`/`onResolve`/`note`).
- **Closed `tradeAsset` union** (rules-engine decision #7): `warriors`+`amount` | `spirit`+`amount` | `item`+`itemRef` | `companion`+`companionId`. **Virtues and corruptions are untradeable by construction** (absent from the union). Wired into `action.trade` (`from`/`to`/`give[]`/`receive[]`).
- **MVP per-kind node props** added to `node.allOf`: `action.trade`/`banner`, `dungeon.subflow`, `event.companion`, `winloss.mainGoal`/`winCondition`/`lossCondition`, `cond.check`/`branch`, `trigger.schedule`/`onState`, `media.narration`/`showImage`.

### 14.3 Defaults, not constraints

Foe `strike`/`movement` and every `battleDefs` card field are **author-editable defaults**, not engine-enforced rules. Per Official App Analysis v0.3 §6, card-deck composition, strike cadence, and movement are app-internal data the content repo explicitly TODOs out — *observed, unverified*. The schema therefore admits these fields without enum-locking deck size or keyword structure, so an author may edit them freely. The one grounded fact (cards drawn per battle = foe level) is engine-owned, not encoded here.

### 14.4 Conformance

The suite grows **12 → 38 cases** (26 new v0.4 cases across TradeAsset, `battleDefs` cards, foe `strike`/`movement`, quest `requirements`, `dormantKingdoms`, room requirements, and the new node-props). All 38 pass; the schema compiles clean under ajv strict mode, draft 2020-12.

### 14.5 Still deferred (by design or by block)

- **Non-MVP node kinds** keep open `props` — v0.4 is MVP-scoped first; later passes specify the rest.
- **Rich hero data** (`library.heroes.overrides`) — content-blocked (CR-7.13.2; not in UDT).
- **`board.mutate` / `BoardState` shape** — repo-blocked (Board unpublished).
- **Relay R1–R11 review** — repo-blocked (Relay unpublished).
- **goal→companion mapping** — content-blocked (quests.md TODO); gates the goal↔ally consistency check.

### 14.6 Reconstruction caveat

The standalone `scenario.schema.json`, `fixtures.js`, and conformance harness were **rebuilt** for this pass (the canonical files were not in hand). They are self-consistent and green, but will differ in incidental ways (ids, the exact `base` fixture shape) from the canonical copies. Reconcile against the repo's files when they are available; the *structure* and the v0.4 contract are what this document fixes.
