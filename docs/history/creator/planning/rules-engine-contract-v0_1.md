...
**v0.1 · Draft for review · The contract for the shared rules engine that both apps run · Last updated: 2026-06-24**
...
| **Per hero** | `warriors`, `spirit`, `corruption` (0–2; a would-be 3rd ends the game), `virtues` (3 active + 3 inactive, which are active), `items` (`gear` set of ≤6 distinct types · `treasure` ≤4 · `potions` ∞ · `questItems` ∞), `companions`, per-hero counter tokens (e.g. Spore). Hero **rich stats** (move value, banner, 7+1 start, 3+3 virtue identities) are *injected content* today (§10.3). |
...
| **Flags / counters** | `scenarioFlags[name]`, `scenarioCounters[name]`, the `sealsRemoved` counter, quest / main-goal completion, monthly companion+adversary quest state, dungeon **cleared-rooms** (per dungeon; persists across heroes). |
...
### 3.4 Tower mirror — read-only, never authored

Skull counter, seals, glyphs (+ facing), drum positions, and calibration come from the tower via the Relay. The engine holds a **read-only mirror** updated from observed inputs (§5.4). It is never the engine's ground truth: the engine *acts* (triggers rotation, seal break, skull drop) and *reacts* to what the tower reports — most importantly the emergence count, which the engine never predicts (the skull invariant, §1.2).
...
| `seal.remove` | `sealsRemoved += 1`; the *which seal* + any emergent skulls + revealed glyph arrive as observed input; pair with a `tower.op` `seal.break` for hardware | fires onState event · (paired `tower.program`) |
| `seal.replace` | Inverse: mark a seal replaced / `sealsRemoved -= 1` | (paired `tower.program`) |
| `flag.set {name, value}` | `scenarioFlags[name] = value` | — |
| `counter.set {name, value}` | `scenarioCounters[name] = value` | — |

> **`seal.remove` (effect) vs. `seal.break` (tower op).** The **effect** mutates the game-state seal model (`sealsRemoved`, which seal). The **tower op** commands the hardware. A "remove a seal" moment typically pairs both; the engine increments the state model and then waits for the tower's glowing-lights indicator (which seal) + emergence count to arrive as observed input. Same split applies to `seal.replace`.
...
- **Glyph gate** (`cond.glyphGate`): while a revealed glyph faces the acting hero's home kingdom, the matching action (Banner/Quest/Battle/Reinforce/Cleanse) requires 1 spirit, else it is blocked. Glyph facing is read from the tower mirror (§3.4).
...
| --- | --- | --- |
| `tower.program` | **Relay** | A resolved tower program: one or more `towerOp`s and/or a `timeline` with `wait`s on the 50 Hz tick. Idempotent / full-state where the protocol allows. **Never** an emergence count. |
...
| Input | Meaning |
| --- | --- |
| `decision` | A player's response to a `ui.prompt` / `InputRequest`: selected option id, target selections (which foe / building / space / card), Advantage spends, quantities, mutual-consent trade terms. Keyed by `InputRequest.id`. |
...
| `sealIndicated` | Tower (glowing lights) | *Which* seal a "remove a seal" event indicates. |
| `glyphRevealed` | Tower | Which glyph appears + its facing. |
| `towerState` | Tower via Relay | Drum positions, calibration, full-state resyncs after a drop. |
...
- **The engine seed is *not* the official game seed.** `meta.provenance.importedSeed.seed` is the official 12-char seed and is **provenance only** — its RNG section is never consumed (Creator §8). The engine's PRNG seed is a **separate runtime seed** established at `init(opts.seed)`, stored in `EngineState`, advanced **only** by the engine, and serialized with every checkpoint so save/resume and replay survive intact. Conflating the two is a bug; this contract forbids wiring the official seed into the engine PRNG.
...
- **Algorithm.** The PRNG implementation lives in the shared `engine` package, so both consumers use the same bytes by construction. The specific algorithm (an engine-local PRNG vs. reusing UDT's `SystemRandom`) is **open** (§10.2); the only hard requirement is that it be fully specified and identical across consumers.
...
- **`SUPPORTED_SCHEMA_RANGE`** — the engine declares the `schemaVersion` range it can execute (e.g. engine ↔ `0.3.x`). At `init`, if `scenario.schemaVersion` is outside the range → **hard fail at load** with a clear compatibility error. Never mid-game.
- **`meta.pins.*` check** — at `init`, `adapters` confirm the *running* ecosystem packages (UDT/Board/Display/Relay-client, pinned at the workspace) satisfy the scenario's pins. A mismatch (e.g. scenario pinned to a UDT older/newer than the workspace) is a **load-time compatibility error**, surfaced clearly (Player §7.1.2).
- **Error behavior, uniformly:** fail at load, fail clearly (structured error codes via `adapters`), never partially run, never silently downgrade. Mid-game version surprises are the exact failure mode the four-axis scheme exists to prevent.
...
1. **Month-end resolution policy.** `randomInRange` (default) vs `triggerAdvanced` are both in `setup.monthEnd`. The *within-range algorithm* (how `randomInRange` picks the end turn between `minTurn` and `maxTurn`) is undecided (schema §13.1; Player §11.1). The contract fixes only the invariants: bounded by the range, deterministic from seed + input stream, identical in both consumers.
2. **PRNG choice & seed wiring.** Engine-local PRNG vs. UDT's `SystemRandom` (the C# replica). The official-game seed (`meta.provenance`) must **never** seed the engine PRNG — confirm and lock. RNG state serializes inside `EngineState`; confirm the draw-point set is exhaustive.
...
8. **Main-goal companion vs. seed ally.** Rules pair the main goal with a companion; whether that equals the seed's `ally` (`setup.selections.allyId`) or is distinct affects setup wiring (schema §13.5). Confirm before the engine treats them as one.
9. **Mutual-consent trade input.** Trades among heroes on the same space require all parties to agree (warriors/spirit/items/companions only; never virtues or corruptions). The multi-party `decision` shape for simultaneous consent needs definition.

---

*Companion to: Canonical Scenario Schema v0.3 (`scenario.schema.json` @ `schemaVersion 0.3.0`, `scenario-schema-v0_3.md`), Player PRD v0.3 (§6.1, §7, §8), Creator PRD v0.3 (§7.12, §8), Node Block Catalog v0.3 (§3–§8). Verified against `ultimatedarktower` v4.1.0 (2026-06-24). Supersedes: nothing (new document).*
