...
async function revealPicksOnTower() {
  ...
  const topGlyph = GLYPHS[GameState.TowerPicks[0] as keyof typeof GLYPHS] ?? {
    side: GameState.TowerPicks[0].split(' ')[1],
  };
  const middleGlyph = GLYPHS[GameState.TowerPicks[1] as keyof typeof GLYPHS] ?? {
    side: GameState.TowerPicks[1].split(' ')[1],
  };
  const bottomGlyph = GLYPHS[GameState.TowerPicks[2] as keyof typeof GLYPHS] ?? {
    side: GameState.TowerPicks[2].split(' ')[1],
  };

  // lights
  const doorwayLights = getDoorwayLightsCommand();
  const lights: Lights = { doorway: doorwayLights };

  // sound
  const sound = getScoringSound();

  // The core library removed the old single-shot MultiCommand(rotate, lights,
  // sound), so send the rotation (carrying the scoring sound) and the
  // doorway-light result as two sequential commands instead.
  await DarkTower.Rotate(
    topGlyph.side as TowerSide,
    middleGlyph.side as TowerSide,
    bottomGlyph.side as TowerSide,
    sound,
  );
...
// Vite bundles this entry as an ES module, so these top-level declarations are
// module-scoped, not global. Expose every function index.html references via its
// inline `onclick=` handlers so they resolve at runtime.
...
// Initial page setup. This runs on module load (a deferred module executes after
// the DOM is parsed), replacing the trailing inline <script> the standalone HTML
// used to carry — which would now run *before* this deferred module and throw.
setDifficultyNormal();
populateSelections();
resetScore();
