...
audioEnableButton.addEventListener('click', async () => {
  // Probe-fetch a known sample so we surface fetch/CORS failures to the visible
  // status banner. We use a display-provided sample URL (emitted by the
  // ultimatedarktowerdisplay bundle) rather than a bundle-relative path, since
  // the audio assets are resolved and emitted by the display package itself.
  // The display package's TowerSampleAudio logs these to console.error only —
  // invisible to the user.
  const probeUrl = CALIBRATION_SOUND_URL;
  try {
