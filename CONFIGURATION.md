...
| --------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tsconfig.base.json`        | `ES2022` / `ESNext` / `bundler`           | Vite-bundled apps (`controller`, `game`, `seed`, `relay-electron`, and the `creator`/`digital`/`player`/`sync` app+node pairs) and all seven private `@udtc/*` + `scenario-store` packages | The default — browser/Vite-consumed TS, strict + `noUnusedLocals`/`noUnusedParameters`/`noFallthroughCasesInSwitch`.                                                                                                                                                                                                                                                        |
...
- A `test` script with **zero test files** (`vitest run --passWithNoTests`) is a deliberate
  placeholder in a handful of packages (`relay-shared`, `creator-card-render`, `creator-theme`, `assets`,
...
| Plain `tsc --build`                       | `relay-{shared,core,client}`, `apps/relay-cli`, `creator-engine`                       | CJS library builds via project references.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `tsc` + hand-rolled `esbuild`             | `core`, `game-data`                                                                    | A `tsc` CJS build plus a separate `esbuild --bundle` ESM bundle (`dist/esm/...mjs`) per entry point, exposed via the `exports` map's `import` condition. `game-data` needed this because Vite/esbuild's dev-mode CJS interop can't statically resolve named exports through tsc's `__exportStar` re-export helper (`export * from` in `src/index.ts`) — a plain `require()`-only consumer never hit it, but a browser dev server serving the linked workspace package raw did (`import { GLYPHS } from 'ultimatedarktowerdata'` came back "does not provide an export named"). `core`'s version additionally needs a `browser` condition to strip its `createRequire` banner and stub `@stoprocent/noble`; `game-data` has neither concern (no dynamic requires, zero runtime deps), so its ESM bundle needs no banner-stripping and no separate `browser` condition — just `import`. |
| No-op / source-only                       | `creator-adapters`, `creator-card-render`, `creator-theme`, `scenario-store`, `assets` | `exports` point straight at `.ts` source; `build` is a placeholder `echo`. `assets` is the exception that also commits hand-written `.d.ts` under `types/` — its `exports` put a `types` condition ahead of `default` so an _emitting_ consumer (`display`, `rootDir: "src"`) can import it without TS6059. See `packages/assets/CLAUDE.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
