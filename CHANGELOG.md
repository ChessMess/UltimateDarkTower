...
- **`MONUMENTS` roster + `Monument` type** — the 8 Covenant monuments (Arch of the Golden Sun, Argent Oak, Cenotaph of the First Prophet, Colossus of Bjorn, Endless Necropolis, Moonstone Temple, Nightmare Cage, Tower Shard) as a reference roster (alphabetical; not seed-encoded). Exported from `udtSeedParser` alongside the other rosters and re-exported by `ultimatedarktowerboard` for its editing-UI monument palette (decision #10/#11).
