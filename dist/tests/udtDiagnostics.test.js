...
const udtDiagnostics_1 = require("../src/udtDiagnostics");
...
describe('UdtDiagnosticsRecorder', () => {
    describe('disabled (default)', () => {
        let recorder;
        beforeEach(() => {
            recorder = new udtDiagnostics_1.UdtDiagnosticsRecorder({ enabled: false });
        });
    ...
    describe('enabled', () => {
        ...
        let sink;
        beforeEach(() => {
            sink = new udtDiagnostics_1.InMemorySink();
            recorder = new udtDiagnostics_1.UdtDiagnosticsRecorder({ enabled: true, sinks: [sink] });
        ...
        test('battery history is separate from events ring', () => {
            for (let i = 0; i < 5; i++)
                recorder.recordBattery(3500 + i, 75 + i);
            ...
            expect(eventsAfterBattery.find(e => e.kind === 'battery_heartbeat')).toBeUndefined();
        });
        ...
        test('capturePayloads off (default) does not include payloadHex', () => {
            ...
            expect((_a = cmdEvent.data) === null || _a === void 0 ? void 0 : _a.payloadHex).toBeUndefined();
        });
        test('capturePayloads on includes hex with truncation', () => {
            ...
            const events = recorder.getRingBuffer();
            const cmdEvent = events.find(e => e.kind === 'cmd_sent');
            expect((_a = cmdEvent.data) === null || _a === void 0 ? void 0 : _a.payloadHex).toMatch(/\.\.\(\+\d+\)/);
            expect((_b = cmdEvent.data) === null || _b === void 0 ? void 0 : _b.payloadLen).toBe(100);
        });
        ...
        test('recordIncident appends a disconnect event with cause', () => {
            ...
            expect((_a = last.data) === null || _a === void 0 ? void 0 : _a.cause).toBe('gatt_health_check');
        });
        ...
        test('a failing sink does not block other sinks', () => {
            const failingSink = {
                onIncident: () => { throw new Error('boom'); },
            };
    ...
    describe('bytesToHex', () => {
        test('encodes and pads short payloads', () => {
            expect((0, udtDiagnostics_1.bytesToHex)(new Uint8Array([0x00, 0x0a, 0xff]))).toBe('000aff');
        });
        test('truncates with marker', () => {
            const data = new Uint8Array(40);
            const hex = (0, udtDiagnostics_1.bytesToHex)(data, 8);
...
describe('UltimateDarkTower diagnostics integration', () => {
    test('default constructor has diagnostics disabled', () => {
        const tower = new UltimateDarkTower_1.default({ adapter: new MockBluetoothAdapter_1.MockBluetoothAdapter() });
    ...
    test('opting in produces an incident on adapter disconnect', async () => {
        const adapter = new MockBluetoothAdapter_1.MockBluetoothAdapter();
        const sink = new udtDiagnostics_1.InMemorySink();
    ...
    test('bt_unavailable cause fires when bluetooth becomes unavailable mid-session', async () => {
        ...
        expect(incident === null || incident === void 0 ? void 0 : incident.cause).toBe('bt_unavailable');
    });
    test('user_initiated cause fires on explicit disconnect()', async () => {
        ...
        expect((_a = sink.list().pop()) === null || _a === void 0 ? void 0 : _a.cause).toBe('user_initiated');
    });
    ...
    test('warn/error log messages are bridged into the events ring', async () => {
        ...
        expect((_a = logEvent.data) === null || _a === void 0 ? void 0 : _a.level).toMatch(/warn|error/);
    });
    test('command queue snapshot is captured at incident time (in-flight cmd preserved)', async () => {
        ...
        expect(incident.commandQueue.currentCommand !== null ||
            incident.commandQueue.queueLength > 0).toBe(true);
    });
});
...
//# sourceMappingURL=udtDiagnostics.test.js.map