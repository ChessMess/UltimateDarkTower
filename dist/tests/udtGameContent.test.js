 ...
 */
const udtGameContent_1 = require("../src/data/udtGameContent");
describe('HEROES (gameplay content)', () => {
    test('has 10 heroes and the keyed/list views agree', () => {
        expect(Object.keys(udtGameContent_1.HEROES)).toHaveLength(10);
        expect(udtGameContent_1.heroes).toHaveLength(10);
    });
    test('each hero has 2 default + 3 unlockable virtues and a banner action', () => {
        for (const hero of udtGameContent_1.heroes) {
    ...
    test('key matches the hero name', () => {
        for (const [key, hero] of Object.entries(udtGameContent_1.HEROES)) {
...
describe('FOES / ADVERSARIES (gameplay content)', () => {
    test('12 foes at levels 2–4', () => {
        expect(udtGameContent_1.foes).toHaveLength(12);
        expect(Object.keys(udtGameContent_1.FOES)).toHaveLength(12);
        for (const foe of udtGameContent_1.foes) {
            expect([2, 3, 4]).toContain(foe.level);
        }
    });
    test('8 adversaries, all level 5', () => {
        expect(udtGameContent_1.adversaries).toHaveLength(8);
        expect(Object.keys(udtGameContent_1.ADVERSARIES)).toHaveLength(8);
        for (const adv of udtGameContent_1.adversaries) {
...
describe('COMPANIONS', () => {
    test('10 companions, each with a title', () => {
        expect(udtGameContent_1.companions).toHaveLength(10);
        expect(Object.keys(udtGameContent_1.COMPANIONS)).toHaveLength(10);
        for (const c of udtGameContent_1.companions) {
...
describe('KINGDOM_VIRTUES', () => {
    test('keyed by the four directions', () => {
        expect(Object.keys(udtGameContent_1.KINGDOM_VIRTUES).sort()).toEqual(['East', 'North', 'South', 'West']);
        expect(udtGameContent_1.kingdomVirtues).toHaveLength(4);
    });
});
//# sourceMappingURL=udtGameContent.test.js.map