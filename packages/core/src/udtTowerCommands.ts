...
export class UdtTowerCommands {
  ...
  private async sendTowerCommandDirect(command: Uint8Array): Promise<void> {
    ...
    try {
      ...
      const maxRetriesReached =
        this.deps.retrySendCommandCount.value >= this.deps.retrySendCommandMax;
      ...
      if (!maxRetriesReached && this.deps.bleConnection.isConnected && !wasCancelled) {
        this.deps.logger.info(
          `retrying tower command attempt ${this.deps.retrySendCommandCount.value + 1}`,
          '[UDT][CMD]',
        );
  ...
  async calibrate(): Promise<void> {
    if (this.deps.bleConnection.performingCalibration) {
      this.deps.logger.warn(
        'Tower calibration requested when tower is already performing calibration',
        '[UDT][CMD]',
      );
      return;
    }
  ...
  async playSound(soundIndex: number): Promise<void> {
    const invalidIndex =
      !Number.isFinite(soundIndex) ||
      soundIndex > Object.keys(TOWER_AUDIO_LIBRARY).length ||
      soundIndex <= 0;
    if (invalidIndex) {
    ...
    const { command } = this.deps.commandFactory.createTransientAudioCommand(
      currentState,
      soundIndex,
      false,
    );
  ...
  private mapLightsToLayerCommands(
    lights: Lights,
  ): Array<{ layerIndex: number; lightIndex: number; effect: number; loop: boolean }> {
  ...
  async lightOverrides(light: number, soundIndex?: number): Promise<void> {
    ...
    if (typeof light !== 'number' || isNaN(light)) {
      this.deps.logger.error(
        `Invalid light parameter: ${light}. Must be a valid number.`,
        '[UDT][CMD]',
      );
      return;
    }
    ...
    if (
      ...
      (typeof soundIndex !== 'number' || isNaN(soundIndex) || soundIndex <= 0)
    ) {
      this.deps.logger.error(
        `Invalid soundIndex parameter: ${soundIndex}. Must be a valid positive number.`,
        '[UDT][CMD]',
      );
      return;
    }
    ...
    if (soundIndex) {
      ...
      const { command, stateWithoutAudio } =
        this.deps.commandFactory.createTransientAudioCommandWithModifications(
          currentState,
          soundIndex,
          false,
          undefined,
  ...
  async rotate(
    ...
    if (soundIndex !== undefined) {
      ...
      if (validSoundIndex) {
        ...
        this.deps.logger.error(
          `attempt to play invalid sound index ${soundIndex} during rotate`,
          '[UDT][CMD]',
        );
      }
    }
    ...
    await this.sendTowerCommand(
      rotateCommand,
      `rotate(${top}, ${middle}, ${bottom}${soundIndex ? `, ${soundIndex}` : ''})`,
    );
  ...
  async breakSeal(seal: SealIdentifier, volume?: number): Promise<void> {
    ...
    this.deps.logger.info(
      `Breaking seal ${seal.level}-${seal.side} - triggering firmware sealReveal animation`,
      '[UDT]',
    );
    await this.lightOverrides(
  ...
  async randomRotateLevels(level: number = 0): Promise<void> {
    ...
    switch (level) {
      ...
      default:
        this.deps.logger.error(
          'Invalid level parameter for randomRotateLevels. Must be 0-6.',
          '[UDT][CMD]',
        );
        return;
    }
    ...
    this.deps.logger.info(
      `Random rotating levels to: top:${topSide}, middle:${middleSide}, bottom:${bottomSide}`,
      '[UDT][CMD]',
    );
  ...
  async setLEDStateful(
    ...
    const currentState = this.deps.getCurrentTowerState();
    const command = this.deps.commandFactory.createStatefulLEDCommand(
      currentState,
      layerIndex,
      lightIndex,
      effect,
    ...
    this.deps.logger.info(
      `Setting LED layer ${layerIndex} light ${lightIndex} to effect ${effect}${loop ? ' (looped)' : ''}`,
      '[UDT][CMD]',
    );
    await this.sendTowerCommand(
      command,
      `setLEDStateful(${layerIndex}, ${lightIndex}, ${effect}, ${loop})`,
    );
  }
  ...
  async playSoundStateful(
    ...
    const clampedVolume =
      volume === undefined ? undefined : Math.min(3, Math.max(0, Math.round(volume)));
    ...
    this.deps.logger.info(
      `Playing sound ${soundIndex}${loop ? ' (looped)' : ''}${clampedVolume !== undefined ? ` at volume ${clampedVolume}` : ''}`,
      '[UDT][CMD]',
    );
    ...
    await this.sendTowerCommand(
      command,
      `playSoundStateful(${soundIndex}, ${loop}${clampedVolume !== undefined ? `, ${clampedVolume}` : ''})`,
    );
  }
  ...
  async rotateDrumStateful(
    ...
    await this.sendTowerCommand(
      command,
      `rotateDrumStateful(${drumIndex}, ${position}, ${playSound})`,
    );
