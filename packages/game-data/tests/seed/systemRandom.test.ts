...
describe('SystemRandom', () => {
  describe('game seed 186022107 (InitializationSeed from AA9A-AAGS-W634)', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        905900453, 632338475, 1126051326, 493750464, 1712952614, 1848245383, 1818683628, 220859879,
        557619172, 892656879, 839241645, 1127367143, 128645597, 1793117455, 611271626, 52411329,
        1689882546, 327620538, 2110791698, 815982906,
      ];
  ...
  describe('game seed 186022106 (QuestSeed from AA9A-AAGS-W634)', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        1931484281, 2226792, 1772469694, 35385261, 743394371, 2119047781, 856489197, 1290984475,
        291939581, 100769927, 1404608531, 1598279286, 799839337, 675877772, 1255577138, 858746564,
        787168317, 1143665769, 2052658486, 2131950400,
      ];
      for (let i = 0; i < expected.length; i++) {
        expect(rng.next()).toBe(expected[i]);
      }
  ...
  describe('seed 0', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        1559595546, 1755192844, 1649316166, 1198642031, 442452829, 1200195957, 1945678308,
        949569752, 2099272109, 587775847,
      ];
      for (let i = 0; i < expected.length; i++) {
        expect(rng.next()).toBe(expected[i]);
      }
  ...
  describe('seed -1 (negative)', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        534011718, 237820880, 1002897798, 1657007234, 1412011072, 929393559, 760389092, 2026928803,
        217468053, 1379662799,
      ];
      for (let i = 0; i < expected.length; i++) {
        expect(rng.next()).toBe(expected[i]);
      }
  ...
  describe('seed 42', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        1434747710, 302596119, 269548474, 1122627734, 361709742, 563913476, 1555655117, 1101493307,
        372913049, 1634773126,
      ];
      for (let i = 0; i < expected.length; i++) {
        expect(rng.next()).toBe(expected[i]);
      }
  ...
  describe('Int32.MaxValue (2147483647)', () => {
    test('Next() sequence matches C#', () => {
      ...
      const expected = [
        1559595546, 1755192844, 1649316172, 1198642031, 442452829, 1200195955, 1945678308,
        949569752, 2099272109, 587775835,
      ];
      for (let i = 0; i < expected.length; i++) {
        expect(rng.next()).toBe(expected[i]);
      }
  ...
  describe('edge cases', () => {
    ...
    test('nextRange throws when min > max', () => {
      const rng = new SystemRandom(42);
      expect(() => rng.nextRange(50, 10)).toThrow(
