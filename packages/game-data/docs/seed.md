...
This package includes a complete encoder/decoder for Return to Dark Tower game seeds, plus a byte-exact replica of .NET's `System.Random` that the game uses for procedural generation.
...
> **v6.0.0:** the seed subsystem moved out of `ultimatedarktower` into this package. Import
> directly from `ultimatedarktowerdata` (or the `ultimatedarktowerdata/seed` subpath) — flat,
> no `seed.*` namespace:
>
> ```typescript
> import { decodeSeed } from 'ultimatedarktowerdata';
> decodeSeed('AA9A-AAGS-W634');
> ```
...
```typescript
import { createSeed } from 'ultimatedarktowerdata';
...
```typescript
import { encodeSeed } from 'ultimatedarktowerdata';
...
```typescript
import { validateSeed, compareSeedsRaw, dumpSeedChars } from 'ultimatedarktowerdata';
...
Imported the same way as the values:
...
import type {
  ...
  Tier1Foe, // also Tier2Foe, Tier3Foe
  Adversary, // also Ally
  Difficulty, // also GameSource, ExpansionType
} from 'ultimatedarktowerdata';
```
...
```typescript
import { SystemRandom } from 'ultimatedarktowerdata';
...
```typescript
import { decodeSeed, SystemRandom } from 'ultimatedarktowerdata';
...
- [board-data.md](board-data.md) — board geometry and hero/monument/foe reference rosters.
