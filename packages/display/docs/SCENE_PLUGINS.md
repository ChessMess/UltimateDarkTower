...
- **`onModelLoaded(info)`** fires once the GLB is in the scene with finalized bounds (`{ root, modelRadius, modelBottomY, modelTopY }`). If the model is already loaded when you attach, it fires immediately. This is the right place to size/position your **content** against the real model. **Camera _framing_ is the exception:** the view fits the camera on model load _before_ this hook runs, so if your plugin drives the camera (via a closure over the view's `applyCameraConfig`, or the view's `camera` constructor option), set it in `attach` (pre-load — the factors are stored and applied by the on-load fit), not here. Setting it in `onModelLoaded` flashes the default framing first. See [`applyCameraConfig`](API.md#applycameraconfigconfig-cameraconfig-void-1).
