...
export const meta = {
  ...
  sources: {
    testingPlan:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-testing-plan.md',
    alternatives:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-alternatives.md',
    perfSkill:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/.claude/skills/darktower-3d-perf/SKILL.md',
    baseline:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/00-baseline.md',
    results:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/RESULTS.md',
  },
};
...
export const baseline = {
  ...
  implementation: {
    loc: null,
    files: null,
    summary: 'No code change — baseline reference.',
    decisionOnScope: 'N/A.',
    knobsChosen: [],
  },
  display: {
  ...
  signals: {
    programs: 30,
    visiblePointLights: 36,
    visibleDirectionalLights: 1,
    bloomEnabled: true,
  },
...
export const alternatives = [
  // ─────────────────────────────────────────── PATH A ───────────────────────────────────────────
  {
    ...
    visualRiskFlag: false,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.18-twelve-lights.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.18-twelve-lights',
    implementation: {
      loc: '+100 / −84',
      files: 3,
      summary:
        'Removed the 12 ring + 12 ledge/base PointLights in Tower3DView.buildLeds. Kept the 12 seal accent PointLights in SealManager.buildSealBacklights. LedRef.redLight is now nullable; LedEffectAnimator and Tower3DView guard the four remaining redLight accesses. Bulk-lights gate state moves from 0↔36 to 0↔12 — prewarm machinery kept intact.',
      decisionOnScope:
        'Scope kept conservative: only per-LED PointLights dropped; seal accents preserved so per-seal locality survives. Proxy meshes + halo sprites for ledge/base LEDs unchanged — those LEDs still look lit, only the cast spill is lost.',
      knobsChosen: [
        'Removed: 12 ring + 12 ledge/base PointLights',
        'Kept: 12 seal accent PointLights',
    ...
    screenshots: null,
    screenshotsNote:
      'No pair — 4.18 is not flagged as visual-difference risk in the testing plan (per §2 step 6).',
  ...
  {
    id: '4.16',
    slug: '4.16-emissive-standard',
    name: 'emissive-standard',
    path: 'A',
    status: 'report-complete',
    shortDescription:
      'Switch proxy material MeshBasicMaterial → MeshStandardMaterial (emissive). Building-block validation only.',
    visualCharacter: 'pixel-level identical to baseline', // src: 4.16-emissive-standard.md Visual capture section
    visualRiskFlag: false,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.16-emissive-standard.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.16-emissive-standard',
    isValidationOnly: true,
    implementation: {
      loc: '+39 / −20',
      files: 3,
      summary:
        'Switched all 24 LED proxy meshes from MeshBasicMaterial to MeshStandardMaterial { color: 0x000000, emissive: <led-color>, emissiveIntensity: 0, toneMapped: false, depthWrite: false }. Driver-write changed from material.opacity to emissiveIntensity. No new config knobs. 1:1 driver mapping.',
      decisionOnScope:
        'NOT a perf candidate — MeshStandardMaterial is marginally more expensive than MeshBasicMaterial per fragment. Success criterion is visual parity + no programs growth + tests pass, NOT frameMs improvement. Exists to confirm the emissive pattern is safe to combine later with 4.1 or 4.18.',
      knobsChosen: [
        'color: 0x000000 + emissive: <led-color> + toneMapped: false (zeros diffuse, emissive-only)',
        'emissiveIntensity = driver.v (1:1 mapping)',
        'depthWrite: false (preserves prior occlusion semantics)',
      ],
    },
    display: {
      ...
      pairs: [
        {
          ...
          caption:
            'Pixel-level identical at Display canvas. Driver-coupled emissive output reproduces the prior MeshBasicMaterial+opacity behavior end-to-end.',
        },
        {
          id: 'retina',
          label: 'Retina full-window — All-LEDs',
          baseline: 'screenshots/4.16-emissive-standard-baseline-retina.png',
          after: 'screenshots/4.16-emissive-standard-after-retina.png',
          caption:
            'Same parity at Retina full-window. The black-albedo + emissive-driven approach is background-independent.',
        },
      ],
    },
  ...
  },
  {
    ...
    status: 'report-complete',
    shortDescription:
      'onBeforeCompile hook injects per-fragment distance² early-exit into the PointLight loop. Keeps 36 PointLights.',
    visualCharacter: 'identical to baseline', // src: 4.11 comparison table
    visualRiskFlag: false,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.2-range-cull.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.2-range-cull',
    implementation: {
      loc: 'single-file change (not quoted as +/−)',
      files: 1,
      summary:
        'Single-file change in Tower3DView.ts. Added buildRangeCulledLightsChunk() + applyPointLightRangeCull(root) helpers. Traverses the loaded GLB and attaches onBeforeCompile to every MeshStandardMaterial (covers MeshPhysicalMaterial too — all 6 unique lit GLB materials). Hook substitutes #include <lights_fragment_begin> with a copy whose PointLight loop wraps each iteration in a distance² guard. Per-material scope; THREE.ShaderChunk never mutated.',
      decisionOnScope:
        'All 6 unique lit GLB materials patched, not just drum-interior — biggest gain comes from the tower body (82 168 triangles dominates screen pixels). Patch returns null if chunk text drifts; degrades to baseline perf rather than producing a broken shader.',
      knobsChosen: [
        'Per-material onBeforeCompile (no global ShaderChunk mutation)',
        'Chunk-text validation guards on the 0.184.0 source',
        'castShadow: false for both light types — no shadow-map cost added',
      ],
    },
    display: {
    ...
    screenshots: null,
    screenshotsNote:
      'No pair committed — 4.2 is perf-only by construction. Sanity screenshot captured during session (no visible range-cull edges) but not committed (8.3 MB).',
  ...
  {
    id: '4.5',
    slug: '4.5-light-probe',
    name: 'light-probe',
    path: 'A',
    status: 'report-complete',
    shortDescription:
      'Drop all 36 PointLights; add one LightProbe (SH3) updated analytically per-frame from 12 emitter positions.',
    ...
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.5-light-probe.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.5-light-probe',
    implementation: {
      loc: '+494 / −156',
      files: '4 modified + 2 new',
      summary:
        'NEW LightProbeManager.ts owns the single THREE.LightProbe. accumulateSH3FromDirectional() projects one directional radiance into 9 SH3 coefficients (Ramamoorthi-Hanrahan analytical irradiance projection). update() sums contributions from the 12 lit emitters each frame. SealManager + Tower3DView drop their PointLights. New unit-test file LightProbeManager.test.ts covers SH math (7 tests).',
      decisionOnScope:
        'All 36 PointLights dropped (not just 12 seal accents). §8.4 requires visiblePointLights → 0; keeping the 24 ring/ledge/base reds would prevent that. §4.18 result showed those 24 cast mostly outward — negligible interior spill contribution.',
      knobsChosen: [
        'LightProbe added at scene init (USE_LIGHT_PROBES define in program key from first compile)',
        'SH update O(N=12) on CPU per frame: ~360 FLOPs',
        'Per-fragment cost: O(1) — 9-term SH dot product regardless of N',
      ],
    },
    display: {
    ...
    screenshots: {
      pairs: [
        {
          id: 'allon-retina',
          label: 'All-LEDs / Retina full-window',
          baseline: 'screenshots/4.5-light-probe-baseline.jpeg',
          after: 'screenshots/4.5-light-probe-after.jpeg',
          caption:
            'Per-seal hot spots replaced by a smooth global SH wash. Bulbs + halos (bloom layer) unchanged. The 9 SH terms cannot reproduce sharp localized peaks — only the smoothly varying integral.',
        },
      ],
    },
    tests: {
      result: 'pass',
      count: '285/285 (17 suites; +1 new LightProbeManager suite, 7 tests)',
      touched: [
        'tests/unit/Tower3DView.test.ts',
        'tests/__mocks__/three.js',
        'tests/unit/LightProbeManager.test.ts (NEW)',
      ],
    },
    interpretation:
  ...
  {
    id: '4.1',
    slug: '4.1-hdr-proxies',
    name: 'hdr-proxies',
    path: 'B',
    status: 'report-complete',
    shortDescription:
      'Drop all 36 PointLights. Push proxy/halo color into HDR (×3.0) and raise bloom threshold to 1.0.',
    visualCharacter: 'none (only bloom-amplified bulbs)', // src: 4.11 comparison table
    visualRiskFlag: true,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.1-hdr-proxies.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.1-hdr-proxies',
    implementation: {
      loc: '+259 / −172',
      files: 8,
      summary:
        'New HDR_PROXY_SCALE = 3.0 constant + applyHdrColor() helper. DEFAULT_LIGHTING.scene.bloom.threshold flipped 0.0 → 1.0 (the load-bearing architectural change). All 24 ring + ledge/base + 12 seal accent PointLights dropped. Proxy/halo material colors HDR-scaled. Driver-coupled opacity unchanged — what newly carries opacity over the bloom threshold is the HDR-scaled color.',
      decisionOnScope:
        'All 36 PointLights dropped (same as 4.5). §8.5 success criterion is "frameMs.median at v-sync cap (≤ 16.7 ms) on Retina full-window" — unambiguous; keeping any PointLights would not achieve it.',
      knobsChosen: [
        'HDR_PROXY_SCALE = 3.0 (peak red = linear(0xff2020).r × 3.0 = 3.0 well above bloom threshold)',
        'bloom.threshold = 1.0 (only HDR-bright pixels bloom)',
        'bloom.strength / radius / resolutionScale unchanged',
      ],
    },
    display: {
      ...
      pairs: [
        {
          ...
          caption:
            'Ledge/base bulbs still pop via HDR bloom. Seal cutouts visible as small red dots. Drum-interior atmospheric spill is GONE — the §4.1 "key tradeoff" reproduced exactly.',
        },
      ],
    },
    tests: {
      result: 'pass',
      count: '279/279',
      touched: ['tests/unit/Tower3DView.test.ts', 'tests/__mocks__/three.js'],
    },
    interpretation:
  ...
  {
    id: '4.4',
    slug: '4.4-two-directional',
    name: 'two-directional',
    path: 'B',
    status: 'report-complete',
    shortDescription:
      'Drop all 36 PointLights. Add 2 red DirectionalLights inside the drum (+X/−X). Continuous-driven by max(driver.v).',
    visualCharacter:
      'uniform global wash from 2 opposing directionals (no hot spots, but bright across the whole body — peaks at any LED on)', // src: 4.11 comparison table
    visualRiskFlag: true,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.4-two-directional.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.4-two-directional',
    implementation: {
      loc: '+374 / −165',
      files: 5,
      summary:
        'All 36 LED PointLights dropped. 2 red DirectionalLights at (±modelRadius × 2, 0, 0), target origin, castShadow: false, visible: true always. Continuous-drive: updateLightsGate writes max(driver.v) × cfg.intensity to both directionals every frame — breathing-spill character. New __testables.getInteriorLights helper for unit tests.',
      decisionOnScope:
        'Continuous-drive (intensity every frame) over binary-gate (visible flip). Sidesteps recompile-trap entirely — count stable at 3 forever (1 white scene.key + 2 red interior). Produces the breathing-spill character §4.4 advertises.',
      knobsChosen: [
        'Positions: +X / −X opposing pair at ±modelRadius × 2 (covers cylindrical drum walls)',
        'Intensity scale = cfg.intensity = 2 (unchanged from sealBacklights.intensity baseline)',
        'castShadow: false (no shadow-map work added)',
      ],
    },
    display: {
      ...
      pairs: [
        {
          ...
          caption:
            'Entire drum body actively lit red — no per-seal hot spots but a uniform global wash. Bulbs/halos still pop via bloom. Recovers a full atmospheric-spill character via globals.',
        },
        {
          id: '1led-retina',
          label: '1-LED / Retina full-window',
          baseline: 'screenshots/4.4-two-directional-1led-baseline.jpeg',
          after: 'screenshots/4.4-two-directional-1led-after.jpeg',
          caption:
            'Defining tradeoff: ONE LED lights the entire drum body. Per-seal locality gone (the body looks the same as in the All-LEDs shot); reads as "fire inside the tower" even when a single LED breathes.',
        },
      ],
    },
    tests: {
      result: 'pass',
      count: '283/283 (16 suites; +5 new tests)',
      touched: ['tests/unit/Tower3DView.test.ts', 'tests/__mocks__/three.js'],
    },
    interpretation:
  ...
  {
    id: '4.19',
    slug: '4.19-interior-sprites',
    name: 'interior-sprites',
    path: 'B',
    status: 'report-complete',
    shortDescription:
      'Drop all 36 PointLights. Add 1–3 large additive interior Sprites per seal, reusing the existing halo recipe.',
    visualCharacter: 'per-seal localised additive blobs + bloom amplification', // src: 4.11 comparison table
    visualRiskFlag: true,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.19-interior-sprites.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.19-interior-sprites',
    implementation: {
      loc: '+375 / −157',
      files: 6,
      summary:
      ...
      decisionOnScope:
        'All 36 dropped (matches §4.5/§4.1/§4.4 floor). "Pairs naturally with 4.1" + "with 4.11" framing assumes zero-PointLights baseline. Dropping only 12 seal accents would have landed in ~50 ms/frame zone at Retina (superlinear scaling).',
      knobsChosen: [
        'enabled: false (default; opt-in via applyLightingConfig)',
        'count: 2 (vertically distributed ±0.15 × modelRadius)',
        "sizeFactor: 0.6 (vs halo's 0.3)",
        'opacity: 0.5 (mid-range — saturates at All-LEDs; decision-time knob)',
        'renderOrder: 1 (between drum body 0 and proxy/halo 2/3)',
      ],
    },
    display: {
    ...
    programsStability: {
      iters: 3,
      samples: [22, 22, 22, 22, 22, 22, 22],
      stable: true,
      note: 'visibleSprites swung 0 → 48 between samples; programs never moved — confirms sprites are OUTSIDE the lights shader hash.',
    },
    screenshots: {
      pairs: [
        {
          ...
          caption:
            'Drum body enveloped in bright red atmospheric glow from 24 additive sprite blobs (12 seals × 2). At default opacity 0.5 the All-LEDs case saturates; decision-time knob: drop to ~0.25.',
        },
        {
          id: '1led-retina',
          label: '1-LED / Retina full-window',
          ...
          caption:
            "Glow originates from the north-top seal cutout only — per-seal locality preserved. The §4.19 vs §4.4 axis: 4.4's 1-LED lights the entire drum; 4.19's lights only the local sprite column.",
        },
      ],
    },
    tests: {
      result: 'pass',
      count: '284/284 (16 suites; +6 new tests)',
      touched: ['tests/unit/Tower3DView.test.ts'],
    },
    interpretation:
  ...
  {
    id: '4.11',
    slug: '4.11-min-cost-combo',
    name: 'min-cost-combo',
    path: 'B',
    status: 'report-complete',
    shortDescription:
      '§4.1 + §4.4 + §4.8 fused. 0 PointLights, 2 interior DirectionalLights, BLOOM PIPELINE DISABLED. Baseline-color proxies (NOT HDR-scaled).',
    visualCharacter:
      'global directional wash NO BLOOM — flat-bright bulbs + un-amplified halos; most extreme departure from baseline aesthetic', // src: 4.11 comparison table
    visualRiskFlag: true,
    sourceFile:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/blob/main/docs/lighting-experiments/4.11-min-cost-combo.md',
    branchUrl:
      'https://github.com/ChessMess/UltimateDarkTowerDisplay/compare/main...lighting/4.11-min-cost-combo',
    implementation: {
      loc: '+340 / −161',
      files: 6,
      summary:
        'Single load-bearing flip: DEFAULT_LIGHTING.scene.bloom.enabled = false. The existing conditional at Tower3DView.ts:1093 already null-checked bloom.enabled before constructing BloomManager — flip makes bloomManager === null after initScene. All 36 PointLights dropped. 2 interior DirectionalLights added (identical to 4.4 recipe). Proxy/halo materials UNCHANGED from baseline (NOT HDR-scaled — without bloom, > 1.0 colors clamp on display).',
      decisionOnScope:
        'All 36 dropped + bloom dropped + proxy color BASELINE (not HDR-scaled). §8.8 "bright MeshBasicMaterial proxies" confirmed with user as baseline-color + toneMapped: false (the existing baseline recipe), NOT 4.1\'s HDR-scaled color.setRGB(2, 0, 0). Keeps §4.11-vs-§4.4 perf comparison clean — only delta vs §4.4 is the bloom drop.',
      knobsChosen: [
        'DEFAULT_LIGHTING.scene.bloom.enabled = false (the single load-bearing flip)',
        '2 DirectionalLights at ±modelRadius × 2 on X (identical to 4.4)',
        'Intensity scale = cfg.intensity = 2 (identical to 4.4)',
        'Proxy/halo materials unchanged from baseline',
      ],
    },
    display: {
    ...
    programsStability: {
      ...
      stable: true,
      note: 'visibleSprites swung 0 → 24; programs never moved; bloomMgr stayed false throughout — pipeline genuinely never constructed.',
    },
    screenshots: {
      pairs: [
        {
          ...
          caption:
            'Drum body uniformly red-washed across entire surface. Bulbs/halos visible but FLAT — no bloom-amplified aura. Most extreme aesthetic departure in the bake-off.',
        },
        {
          id: '1led-retina',
          label: '1-LED / Retina full-window',
          ...
          caption:
            'Whole drum bathed in uniform red wash from 2 directionals — even with just one LED active. NO per-LED locality information in the spill pattern. Same as §4.4 but flatter un-amplified bulb character.',
        },
      ],
    },
    tests: {
      result: 'pass',
      count: '287/287 (16 suites; +9 new tests)',
      touched: ['tests/unit/Tower3DView.test.ts', 'tests/__mocks__/three.js'],
    },
    interpretation:
...
export const decisionFramework = [
  {
    ...
    because:
      '4.2: "identical to baseline" (4.11 comparison-to-prior-leaders table). 4.18 loses only corner-positioned PointLights that "cast mostly outward — contribution to interior spill is negligible" (4.18 observations).',
  ...
  },
  {
    ...
    runner: null,
    because:
      '"smooth global SH wash (subtle, no hot spots) … the drum body reads as gently lit; closest to baseline\'s atmospheric feel" (4.5 + 4.19 comparison tables). O(1) per-fragment via 9-term SH dot product.',
  ...
  },
  {
    ...
    runner: null,
    because:
      '"per-seal localised additive blobs … you can tell from the spill alone which LED is lit, unlike §4.4" (4.19 interpretation). Reuses proven in-tree halo infrastructure; no shader-recompile risk because sprites are outside the lights hash.',
  ...
  },
  {
    ...
    runner: null,
    because:
      '"uniform global wash from 2 opposing directionals … recovers a full atmospheric-spill character via globals … standalone-viable shipping candidate" (4.4 interpretation). Spill character arguably more atmospheric than baseline.',
  ...
  },
  {
    ...
    runner: null,
    because:
      '"the cheapest possible delivery short of dropping all visuals" (4.11 observations). Only entry that drives Retina Empty to v-sync; drawCalls −57%, programs −73%. Useful ONLY if the dropped-bloom look is explicitly approved.',
  ...
  },
  {
    ...
    because:
      '4.16 is a validation building block (no perf claim) and the smallest LOC change; 4.2 is a single-file perf win with zero visual change. Both preserve the existing paradigm.',
  ...
  },
  {
    ...
    runner: null,
    because:
      '4.1 ships zero PointLights but loses all interior spill. The three spill-recovery techniques produce three different visual characters at the same v-sync perf — pick by aesthetic.',
