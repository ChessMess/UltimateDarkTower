...
function horizontalBars(rows, opts = {}) {
  ...
  const scale = opts.log
    ? (v) => labelW + (Math.log10(Math.max(v, xMin)) - Math.log10(xMin)) / (Math.log10(xMax) - Math.log10(xMin)) * plotW
  ...
  for (const t of ticks) {
    const x = scale(t);
    svg.appendChild(el('line', { class: 'grid-line', x1: x, x2: x, y1: padTop, y2: padTop + plotH }));
    svg.appendChild(el('text', { class: 'tick-text', x: x, y: padTop + plotH + 14, 'text-anchor': 'middle' }, String(t)));
  }

  // X axis label
  if (opts.xLabel) {
    svg.appendChild(el('text', { class: 'tick-text', x: labelW + plotW / 2, y: H - 4, 'text-anchor': 'middle', 'fill': 'var(--c-text-2)' }, opts.xLabel));
  }

  // Reference line (e.g., baseline marker)
  if (opts.refLine) {
    const x = scale(opts.refLine.x);
    svg.appendChild(el('line', { class: 'reference-line', x1: x, x2: x, y1: padTop, y2: padTop + plotH }));
    if (opts.refLine.label) {
      svg.appendChild(el('text', { class: 'reference-label', x: x + 4, y: padTop - 4 }, opts.refLine.label));
    }
  }
  ...
  rows.forEach((row, i) => {
    ...
    // Row label (left)
    svg.appendChild(el('text', { class: 'label-text', x: labelW - 8, y: y + rowH / 2 + 4, 'text-anchor': 'end' }, row.label));
    if (row.sublabel) {
      svg.appendChild(el('text', { class: 'label-text small', x: labelW - 8, y: y + rowH / 2 + 14, 'text-anchor': 'end' }, row.sublabel));
    }
    ...
    svg.appendChild(el('text', { class: 'value-text', x: x1 + 8, y: y + rowH / 2 + 4 }, row.valueLabel || fmt(row.value)));
    if (row.deltaLabel) {
  ...
  // Axis baseline
  svg.appendChild(el('line', { class: 'axis-line', x1: labelW, x2: labelW, y1: padTop, y2: padTop + plotH }));
...
export function chartSeq5FpsRetina(all) {
  ...
  const rows = all.map(a => {
    ...
    const delta = a.id === '00' ? null : ((fps - baseline.retina.seq5.fps) / baseline.retina.seq5.fps) * 100;
    return {
      id: a.id,
  ...
  return horizontalBars(rows, {
    ...
    width: 600, rowH: 30, labelW: 200, padR: 110,
    xMin: 0, xMax: 130, xTicks: [0, 30, 60, 90, 120],
...
export function chartPrograms(all) {
  ...
  return horizontalBars(rows, {
    ...
    width: 600, rowH: 30, labelW: 200, padR: 70,
    xMin: 0, xMax: 35, xTicks: [0, 6, 22, 30],
...
export function chartDrawCallsAllLeds(all) {
  ...
  return horizontalBars(rows, {
    ...
    width: 600, rowH: 30, labelW: 200, padR: 110,
    xMin: 0, xMax: 260, xTicks: [0, 87, 187, 235],
