...
| `onZoomChange` | `(distanceFactor: number) => void` | —           | Callback fired when the live camera distance changes by more than a tiny epsilon (per-frame detection) — covers wheel-zoom, orbit-drag zoom, `applyCameraConfig`, and Center/Reset uniformly. `distanceFactor` matches `getLiveCameraFactors().distanceFactor` (`1` = the default fit). |
