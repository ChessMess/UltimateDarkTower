...
| `boardDisc.source`                   | `'image' \| 'procedural'` | `'image'`  | Texture source — see "Image vs procedural" below. `'image'` requires `boardTextureUrl`; without it, falls back to procedural                                                                                                                              |
...
- **`'image'` (default)** — loads the **consumer-supplied `boardTextureUrl`** (see `TowerDisplayOptions`; the package ships the real game-board art at `dist/3d/assets/board.png`) via `THREE.TextureLoader` ([GameBoardImageTexture.ts](../src/3d/GameBoardImageTexture.ts)). **With no `boardTextureUrl` set this behaves exactly like `'procedural'`** — the art is no longer bundled, so there is nothing to load. The texture is configured with `colorSpace = SRGBColorSpace`, the renderer's max anisotropy, `ClampToEdgeWrapping`, and a calibrated `texture.rotation` so kingdom-0 faces +Z. **Async load** — `GroundDiscManager` kicks off the load on first use and falls back to the procedural texture as a temporary stand-in until the image resolves, then swaps `material.map` in place. If the load fails (asset missing or fetch error) the manager logs a warning and permanently falls back to procedural for the session.
