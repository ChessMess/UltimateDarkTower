...
Drop Skull and Clear Skulls buttons; a live counts readout (`getSkullCounts()`, polled every ~250ms — `N total · N tower · N board`, plus `· N transit` / `· N queued` when non-zero) directly below them; a Skull Appearance row (Model + Collider dropdowns — Model defaults to Sphere, Collider is disabled until a model is picked); a Triggers row with an Auto-drop checkbox (drops one skull each time `state.beam.count` increases); an Unstick Skulls row with **Shake Skulls** (`physics.shakeSkulls()`) and **Shake Tower** (`view3D.shakeTower()`) buttons, a Click a skull to shake it checkbox (`skull.clickToShake`), and a Skulls can sleep checkbox (`skull.canSleep`); plus twelve sliders (skull radius, max skulls, friction, restitution, angular/linear damping, drum/seal/static/board friction, solver iterations, shake strength). Drop Skull adds one skull per click up to `skull.maxCount`; Clear Skulls removes every active skull. See [PHYSICS §Unsticking skulls](PHYSICS.md#unsticking-skulls) for what the shake controls do. Dropdowns, checkboxes, buttons, and sliders all write into (or call through) a single `PhysicsConfig` object via `applyPhysicsConfig(partial)` on the handle returned by `attachSkullPhysics` (Shake Tower calls `Tower3DView#shakeTower` directly, since it isn't a config leaf).
