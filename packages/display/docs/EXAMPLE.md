...
Drop Skull and Clear Skulls buttons; a live counts readout (`getSkullCounts()`, polled every ~250ms — `N total · N tower · N board`, plus `· N transit` / `· N queued` when non-zero) directly below them; a second readout below that (`getSkullsBySeal()`, same poll — a `Behind seals — pocket|nearest mode` header line over a row of wrapped chips, one per seal with a nonzero count plus a loose chip for unattributed skulls, open seals highlighted) — see [PHYSICS §Counting skulls by seal](PHYSICS.md#counting-skulls-by-seal); a Skull Appearance row (Model + Collider dropdowns — Model defaults to Sphere, Collider is disabled until a model is picked); a Triggers row with an Auto-drop checkbox (drops one skull each time `state.beam.count` increases); an Unstick Skulls section with **Shake Skulls** (`physics.shakeSkulls()`) and **Shake Tower** (`view3D.shakeTower()`) side by side, and **Shake Stuck Skulls** on its own row below them (`physics.shakeSelectedSkull(ids)` for exactly the ids `getSkullsBySeal()` reports behind an already-broken seal, plus any unattributed/loose ones; its label appends the live count, e.g. `Shake Stuck Skulls (3)`, and it's disabled when that list is empty), a Shake skulls when seal removed checkbox (`seal.shakeSkullsOnSealRemoval`, checked by default — waits `seal.shakeSkullsOnSealRemovalDelaySeconds` after a seal breaks, then auto-shakes whichever of its skulls are still there; see [PHYSICS §Unsticking skulls](PHYSICS.md#unsticking-skulls)), a Click a skull to shake it checkbox (`skull.clickToShake`), and a Skulls can sleep checkbox (`skull.canSleep`); plus fifteen sliders (skull radius, max skulls, friction, restitution, angular/linear damping, drum/seal/static/board friction, solver iterations, shake strength, shake horizontal/upward factors, seal-break delay). Drop Skull adds one skull per click up to `skull.maxCount`; Clear Skulls removes every active skull. See [PHYSICS §Unsticking skulls](PHYSICS.md#unsticking-skulls) for what the shake controls do. Dropdowns, checkboxes, buttons, and sliders all write into (or call through) a single `PhysicsConfig` object via `applyPhysicsConfig(partial)` on the handle returned by `attachSkullPhysics` (Shake Tower calls `Tower3DView#shakeTower` directly, since it isn't a config leaf).
