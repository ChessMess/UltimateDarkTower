...
The trace attributes long frames (GPU vs. compositor vs. JS vs. shader compile); `collectPerfReport` tells you _that_ the frame was long. Different layers — use them together when investigating an attribution question.
...
1. **Baseline capture FIRST** — run the full triplet (Empty / 1-LED / All-LEDs, see below) on `main` _before_ implementing the alternative. Save the report. Without a same-machine same-canvas baseline, "this is faster" claims are noise.
...
**Canvas-size sensitivity.** The dominant cost on most alternatives is per-fragment. Test at the canvas size that _matters_:
