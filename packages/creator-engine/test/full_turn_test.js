...
const fight = (level) => {
  const s = [];
  for (let k = 0; k < level; k++) s.push(bc({ reveal: true }), bc({ resolve: true }));
  return s;
};
...
ok(
  'hero starts at the home citadel (rules.md §Hero Setup)',
  r0.state.heroes.hero1.location === 'Radiant Mountains',
);
ok(
  'the 16-building registry is live',
  r0.state.buildings.length === 16 &&
    r0.state.buildings.every((b) => b.skulls === 0 && !b.destroyed),
);
ok(
  'full-turn heroes carry the 3+3 virtue split (placeholders)',
  r0.state.heroes.hero1.virtues.active.length === 3 &&
    r0.state.heroes.hero1.virtues.inactive.length === 3,
);
ok('month 1 is exactly one turn per player', r0.state.clock.turnsThisMonth === 1);
ok(
  "boardSetup emits a placeHero directive for hero1's home citadel",
  r0.directives.some(
    (d) =>
      d.type === 'board.mutate' &&
      d.command === 'placeHero' &&
      d.args.hero === 'hero1' &&
      d.args.to === 'Radiant Mountains',
  ),
);
ok(
  'the full-turn action menu offers banner/move/heroic/reinforce/endTurn',
...
// ---------- foe levels derive from selection tier ----------
{
  const run = drive(gf, opts, [
    act('move'),
    mov('Dragontooth Lake'),
    act('battle'),
    tgt('dragons'),
  ]);
  const b = lastOf(run).state.clock.battle;
  ok(
    'dragons (tier3) draw 4 battle cards',
    b && b.level === 4 && b.hand.length === 4,
    JSON.stringify(b && b.hand),
  );
  const run2 = drive(gf, opts, [
    act('move'),
    mov('The Tundra'),
    act('battle'),
    tgt('frost-trolls'),
...
// ---------- one full turn: banner + move + heroic (battle) + reinforce, any order ----------
{
  const run = drive(gf, opts, [
    ...
    act('battle'),
    tgt('brigands'),
    ...fight(2), // reveal + resolve each of the 2 drawn cards
  ]); // banner, move, battle only
  const s = lastOf(run).state;
  ok(
    'banner + move + battle in a single turn',
    s.clock.latches.bannerUsed && s.clock.latches.moveUsed && s.clock.latches.heroicActionUsed,
  );
  ok(
    'battle heroic completion awards 2 spirit',
    s.heroes.hero1.spirit === 3,
    'spirit=' + s.heroes.hero1.spirit,
  );
  ok('brigands defeated at their own space', !s.foes.some((f) => f.foeId === 'brigands'));
  ok(
    'turn is still open after three actions (menu shrinks)',
    lastOf(run).status === 'awaitingInput' && lastOf(run).awaiting.id === 'action',
  );
  ok(
    'spent options leave the menu',
    !lastOf(run).awaiting.options.some((o) => ['banner', 'move', 'battle', 'quest'].includes(o.id)),
  );
}

// ---------- per-turn latches fault on reuse ----------
expectFault('second banner in a turn faults', () =>
  drive(gf, opts, [act('banner'), act('banner')]),
);
expectFault('second move in a turn faults', () =>
  drive(gf, opts, [act('move'), mov('Delmsmire'), act('move'), mov('The Tundra')]),
);
...
expectFault('battling a foe elsewhere faults', () =>
  drive(gf, opts, [act('battle'), tgt('brigands')]),
);
expectFault('battling the unspawned adversary faults', () =>
  drive(gf, opts, [act('battle'), tgt('ashstrider')]),
);
...
}
{
  ...
  const h = lastOf(run).state.heroes.hero1;
  ok(
    'village free reinforce grants 6 warriors (7 → 13)',
    h.warriors === 13,
    'warriors=' + h.warriors,
  );
}
{
  ...
  const h = lastOf(run).state.heroes.hero1;
  ok(
    'village enhanced reinforce: 1 spirit → 12 warriors',
    h.warriors === 19 && h.spirit === 0,
    'w=' + h.warriors + ' s=' + h.spirit,
  );
}
{
...
expectFault('reinforce away from any building faults', () =>
  drive(gf, opts, [act('move'), mov('Delmsmire'), act('reinforce')]),
);
...
// ---------- citadel enhanced: 5 spirit → activate a virtue ----------
{
  ...
  const run = drive(gf, opts, [
    ...
    dimp(false),
    dmove('E'),
    dmove('E'), // clears the vault: spirit 1→2→1, +1 quest, +2 heroic = 4
    act('endTurn'),
    act('move'),
    mov('Upper Ice Fangs'),
    act('reinforce'), // sanctuary free: +1 spirit = 5
    act('endTurn'),
    act('move'),
    mov('Radiant Mountains'),
    act('reinforce', { enhanced: true }), // citadel: -5 spirit → virtue
  ]);
  const h = lastOf(run).state.heroes.hero1;
  ok(
    'citadel enhanced activates a virtue for 5 spirit',
    h.virtues.active.length === 4 && h.virtues.inactive.length === 2 && h.spirit === 0,
...
// ---------- Cleanse removes ALL skulls from the building on your space ----------
{
  ...
  ok(
    "cleanse zeroes the building's skulls and returns them to the supply",
    citadel.skulls === 0 && s.skulls.onBoard === 0,
    'skulls=' + citadel.skulls + ' onBoard=' + s.skulls.onBoard,
  );
  ok(
    'cleanse pays the 2-spirit heroic reward',
    s.heroes.hero1.spirit === 3,
    'spirit=' + s.heroes.hero1.spirit,
  );
}
...
// ---------- per-building skulls: the 4th destroys, owner gains corruption ----------
{
  ...
  ok(
    'the 4th skull returns to supply, the 3 leave the game (supply 24-1+1)',
    s.skulls.supply === 24 && s.skulls.onBoard === 0,
...
// ---------- LOSS end-to-end: three razed buildings → third corruption ----------
{
  ...
  const f = lastOf(run);
  ok(
    'goldenFull reaches a CLEAN LOSS (third-corruption)',
    f.status === 'lost' && f.state.outcome.reason === 'third-corruption',
...
// ---------- monthly quests (rules.md §Monthly Quests) ----------
{
  ...
  const run = drive(gf, opts, [
    ...
    act('move'),
    mov('Delmsmire'),
    act('quest', { questId: 'zaida-escort' }),
  ]);
  const s = lastOf(run).state;
  ok(
    'month 2 activates the companion + adversary quests',
    (s.activeQuests || []).some((q) => q.questId === 'adv-quest-m2') || s.quests['zaida-escort'],
    JSON.stringify(s.activeQuests),
  );
...
expectFault('a monthly quest cannot be taken before it is issued', () =>
  drive(gf, opts, [act('move'), mov('Delmsmire'), act('quest', { questId: 'zaida-escort' })]),
); // month 1
expectFault("a quest's location requirement gates the quest action", () =>
  drive(gf, opts, [act('endTurn'), act('quest', { questId: 'zaida-escort' })]),
); // month 2 but at the citadel
{
  ...
  const failed = run
    ...
    .some(
      (d) => d.type === 'log.entry' && d.event === 'questFailed' && d.questId === 'adv-quest-m2',
    );
  ...
  ok(
    'the failed adversary quest makes the world worse (skull placed from supply on a north building)',
    run
      .flatMap((r) => r.directives)
      .some(
        (d) =>
          d.type === 'board.mutate' &&
          d.command === 'placeSkull' &&
          (d.args || {}).source === 'effect' &&
          (d.args || {}).kingdom === 'north',
      ),
  );
  ok(
    'the failed monthly quest is retired',
    !(s.activeQuests || []).some((q) => q.questId === 'adv-quest-m2'),
  );
}
...
const winScript = [
  ...
  act('move'),
  mov("Azkol's Bane"),
  act('quest', { questId: 'azkol-shrine' }),
  act('endTurn'),
  // M2 T3 — the main goal at the Tower → Ashstrider spawns there
  act('move'),
  mov('the-tower'),
  act('quest', { questId: 'recover-azkols-treasures' }),
  act('endTurn'),
...
];
{
  ...
  const f = lastOf(run);
  ok(
    'goldenFull reaches a CLEAN WIN (adversary-defeated)',
    f.status === 'won' && f.state.outcome.reason === 'adversary-defeated',
    f.status +
      '/' +
      f.state.outcome.reason +
      ' m' +
      f.state.clock.month +
      't' +
      f.state.clock.turnInMonth,
  );
  ...
  ok(
    'the shrine relic quest spent its warriors',
    run
      .flatMap((r) => r.directives)
      .some(
        (d) =>
          d.type === 'log.entry' && d.event === 'questComplete' && d.questId === 'azkol-shrine',
      ),
  );
  ...
  const traceOf = (rr) =>
    JSON.stringify(
      rr.map((x) => ({
        s: x.status,
        d: engine.digest(x.state),
        dir: JSON.stringify(x.directives),
      })),
    );
  ok(
    'the WIN stream replays byte-identically (lockstep)',
    traceOf(run) === traceOf(drive(gf, opts, winScript)),
  );
}
...
// ---------- legacy golden is untouched by all of the above ----------
{
  ...
  const win = engine.replay(golden, opts, [
    lact('quest'),
    lobs(0),
    lact('quest'),
    lobs(0),
  ...
  ok(
    'legacy golden still wins via the single-action loop',
    lastOf(win).state.outcome.reason === 'adversary-defeated',
  );
  ...
  ok(
    'legacy heroes carry no placeholder virtues',
    first.state.heroes.hero1.virtues.active.length === 0,
  );
}
