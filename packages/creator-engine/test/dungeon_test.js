...
const noImprove = engine.replay(golden, opts, [
  act('dungeon'),
  dadv(false),
  dmove('E'),
  dmove('E'),
...
check(
  'enter routes the subflow into the entrance room (awaits the improve boundary)',
  () => noImprove[1].awaiting && noImprove[1].awaiting.id === 'dungeonRoomAdvantage',
);
check(
  'entry room inside-event resolves (+1 spirit on enter)',
  () => noImprove[1].state.heroes.hero1.spirit === 2,
);
check('declining the improvement spends no Advantage', () => h(niF.state).advantages === 6);
check(
  'enter-requirement spirit tax is paid at the warded hall (spirit 2 → 1)',
  () => h(niF.state).spirit === 1,
);
check('target room clears the whole dungeon (3 rooms cleared, persist per-dungeon)', () => {
  const cr = niF.state.dungeons[DUN].clearedRooms;
  return (
    cr.length === 3 &&
    cr.includes('vault-entry') &&
    cr.includes('vault-hall') &&
    cr.includes('vault-target')
  );
});
check(
  "clearing the target completes the dungeon's spawning quest",
  () => niF.state.quests['azkol-vault'] && niF.state.quests['azkol-vault'].complete === true,
);
check(
  'target inside-event fired (vaultCleared flag set)',
  () => niF.state.flags.vaultCleared === true,
);
check(
  'control returns to the turn loop after completion (awaits the skull drop)',
...
check('completing a dungeon emits a removeDungeonToken board mutation', () =>
  noImprove
    ...
    .some((d) => d.type === 'board.mutate' && d.command === 'removeDungeonToken'),
);

// ---- full clear WITH the one-time room improvement ----
const withImprove = engine.replay(golden, opts, [
  act('dungeon'),
  dadv(true),
  dmove('E'),
  dmove('E'),
...
check(
  'improving the entry room spends exactly 1 Advantage (6 → 5)',
  () => h(wiF.state).advantages === 5,
);
check(
  'improveOnce effects apply (+2 warriors: 7 base +2 improve +1 hall = 10)',
  () => h(wiF.state).warriors === 10,
);
check(
  'dungeon completes the same way with or without the improvement',
...
check(
  'leaving clears only the resolved room(s), not the dungeon',
  () =>
    lF.state.dungeons[DUN].clearedRooms.length === 1 &&
    lF.state.dungeons[DUN].clearedRooms[0] === 'vault-entry',
);
check('leaving does NOT complete the spawning quest', () => !lF.state.quests['azkol-vault']);
check(
  'leaving returns control to the turn loop (awaits the skull drop)',
  () => lF.awaiting.id === 'skullCounter',
);
...
check(
  "a cleared room is not re-resolved on re-entry (entry's +1 spirit not re-applied)",
  () => h(reF.state).spirit === 1,
);
check(
  'improve-once holds across re-entry (still only 1 Advantage ever spent: 6 → 5)',
  () => h(reF.state).advantages === 5,
);
check(
  'the dungeon is completable across two visits (quest complete after re-entry)',
...
function trace(run) {
  return run.map((r) => ({
    s: r.status,
    d: engine.digest(r.state),
    dir: JSON.stringify(r.directives),
  }));
}
check(
  'dungeon walk is deterministic (byte-identical replays)',
  () =>
    JSON.stringify(
      trace(engine.replay(golden, opts, [act('dungeon'), dadv(true), dmove('E'), dmove('E')])),
    ) ===
    JSON.stringify(
      trace(engine.replay(golden, opts, [act('dungeon'), dadv(true), dmove('E'), dmove('E')])),
    ),
...
expectFault("glyph gate: a block with no 'blocked' port is a fault", () => {
  ...
  s.tower.glyphFacing = { north: 'quest' };
  interpretNode(
    { id: 'g2', kind: 'cond.glyphGate', props: { action: 'quest' }, wires: { out: ['o'] } },
    s,
    [],
  );
});
    ...
    d1: {
      ...
      rooms: [
        ...
        },
        {
          ...
          exits: { W: 'door' },
          enterRequirement: {
            condition: { subject: 'flag', comparator: 'eq', value: true, key: 'haveKey' },
            onFail: [{ op: 'corruption.gain' }],
          },
          insideEvent: [{ op: 'resource.gain', resource: 'warriors', amount: 1 }],
        },
      ],
...
check(
  'enter-requirement spirit block: insufficient spirit → onFail + leave (room NOT cleared)',
  () => {
    const s = dungeonState();
    s.heroes.hero1.spirit = 0; // can't pay the cost of 5
    const r = resolveRoomEntry(wardNode, s, []);
    return (
      r.goto === 'L' &&
      s.heroes.hero1.corruption === 1 &&
      !s.dungeons.d1.clearedRooms.includes('r-ward') &&
      s.clock.dungeon === null &&
      s.heroes.hero1.warriors === 7
    ); // inside-event did NOT run
  },
);
check('enter-requirement condition block: condition unmet → onFail + leave', () => {
  ...
  return (
    r.goto === 'L' &&
    s.heroes.hero1.corruption === 1 &&
    !s.dungeons.d1.clearedRooms.includes('r-cond')
  );
});
check('enter-requirement condition satisfied → room resolves normally', () => {
  const s = dungeonState();
  s.flags.haveKey = true;
  const r = resolveRoomEntry(condNode, s, []);
  // no improveOnce here, not target → awaits the move boundary; inside-event ran (+1 warrior)
  return (
    r.await &&
    r.await.request.id === 'dungeonMove' &&
    s.heroes.hero1.warriors === 8 &&
    s.dungeons.d1.clearedRooms.includes('r-cond')
  );
});
check(
  'cleared rooms persist ACROSS heroes (contract §3.1): a cleared room is not re-resolved for hero2',
  () => {
    const s = dungeonState();
    ...
    s.clock.activeHero = 'hero2';
    s.clock.turnOrder = ['hero1', 'hero2'];
