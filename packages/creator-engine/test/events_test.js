...
// ---------- scheduled events fire at the end of the matching turn ----------
{
  ...
  ok(
    'tower stirs at turn 1 (drum.rotate emitted)',
    dirs(run).some(
      (d) => d.type === 'tower.program' && (d.ops || []).some((o) => o.channel === 'drum.rotate'),
    ),
  );
  ok('new wares refresh the market', lastOf(run).state.market.includes('azkol-idol'));
  ok(
    "no strike on month 1's single turn (turn-2 trigger idle)",
    !dirs(run).some((d) => d.type === 'log.entry' && d.event === 'foeStrike'),
  );
  ok(
    'skull invariant holds across event directives',
    dirs(run)
      .filter((d) => d.type === 'tower.program')
      .every((d) => (d.ops || []).every((o) => !('count' in o))),
  );
}
...
// ---------- foes strike: each foe on the board hits the acting hero ----------
{
  ...
  ok(
    'all three foes strike at turn 2 of the month',
    strikes.length === 3,
    'strikes=' + strikes.length,
  );
  ok(
    'strikes cost the acting hero warriors (7 − 3)',
    s.heroes.hero1.warriors === 4,
    'warriors=' + s.heroes.hero1.warriors,
  );
}
{
  ...
  const run = drive(gf, opts, [
    ...
    tgt('brigands'),
    ...fight(2),
    act('endTurn'), // M1: kill brigands
    act('endTurn'),
    act('endTurn'),
  ]); // M2 T2 strike
  const strikes = dirs(run).filter((d) => d.type === 'log.entry' && d.event === 'foeStrike');
  ok(
    'a defeated foe is skipped by foes-strike',
    strikes.length === 2 && !strikes.some((d) => d.foeId === 'brigands'),
  );
}
...
// ---------- foes grow: the status ladder advances and clamps ----------
{
  ...
  const s = lastOf(run).state;
  ok(
    'foes grow at turn 3 (ready → savage)',
    s.foes.every((f) => f.status === 'savage'),
    JSON.stringify(s.foes.map((f) => f.status)),
  );
  const long = drive(gf, opts, Array(13).fill(act('endTurn'))); // many months of growth
  ok(
    'the ladder clamps at lethal',
    lastOf(long).state.foes.every((f) => ['savage', 'lethal'].includes(f.status)),
  );
}

// ---------- events resolve before the seat rotates (multi-hero) ----------
{
  const run = drive(gf, { seed: 'mvp-runtime-seed', playerCount: 2 }, [
    act('endTurn'),
    act('endTurn'),
    act('endTurn'),
  ]);
  ...
  ok(
    'strikes hit the hero whose turn is ending',
    s.heroes.hero1.warriors === 4 && s.heroes.hero2.warriors === 7,
...
// ---------- onState triggers: raised events are consumed at the turn boundary ----------
{
  ...
  c.graph.nodes.push(
    {
      id: 'n-trig-qdone',
      kind: 'trigger.onState',
      props: { trigger: { on: 'onState', event: 'questComplete' } },
      wires: { out: ['n-ev-qdone'] },
    },
    {
      id: 'n-ev-qdone',
      kind: 'event.towerActs',
      props: { effects: [{ op: 'flag.set', name: 'sagaAdvanced', value: true }] },
      wires: {},
    },
  );
  ...
  const s = lastOf(run).state;
  ok(
    'an onState trigger fires at the boundary after its event was raised',
    s.flags.sagaAdvanced === true,
  );
  const run2 = drive(c, opts, [act('endTurn')]);
  ok(
    'the onState trigger stays idle when its event never fired',
    lastOf(run2).state.flags.sagaAdvanced === undefined,
  );
}
...
// ---------- determinism & legacy inertness ----------
{
  ...
  ok(
    'event-heavy streams replay byte-identically',
    traceOf(drive(gf, opts, script)) === traceOf(drive(gf, opts, script)),
  );
  ...
  ok(
    'legacy golden emits no event directives (no trigger nodes)',
    !dirs(legacy).some(
      (d) =>
        d.type === 'log.entry' &&
        ['foeStrike', 'foesGrow', 'towerStirs', 'newWares'].includes(d.event),
    ),
  );
}
