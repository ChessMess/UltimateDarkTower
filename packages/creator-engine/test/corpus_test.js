...
const {
  ...
  goldenAmpleSupply,
  goldenAuthoredLoss,
  goldenAuthoredLossFlag,
  goldenWardedVault,
  goldenAuthoredLossResource,
  goldenAuthoredLossSeal,
  goldenAuthoredLossFoe,
} = require('../dist/golden-fixture');
...
const cond = (subject, comparator, value, key) =>
  key === undefined ? { subject, comparator, value } : { subject, comparator, value, key };

// per-step (status, state digest, ordered directives) trace — the lockstep comparison unit (§9)
function trace(run) {
  return run.map((r) => ({
    status: r.status,
    digest: engine.digest(r.state),
    directives: JSON.stringify(r.directives),
  }));
}
function identical(scn, opts, inputs) {
  return (
    JSON.stringify(trace(engine.replay(scn, opts, inputs))) ===
    JSON.stringify(trace(engine.replay(scn, opts, inputs)))
  );
}
function lastOf(run) {
...
function dropTriggers(run) {
  return run
    .flatMap((r) => r.directives)
    .filter((d) => d.type === 'tower.program')
...
ok(
  'empty-supply: reaches a clean LOSS',
  esF.status === 'lost',
  'got ' + esF.status + ' / ' + esF.state.outcome.reason,
);
...
ok(
  "empty-supply: loss fires at turn 3's Action: End",
  esF.state.clock.month === 1 && esF.state.clock.turnInMonth === 3,
);
// the loss precedes the drop emit, so only the first two turns emitted a drop trigger:
ok(
  'empty-supply: exactly 2 skull drops occurred (turn 3 lost before dropping)',
  dropTriggers(esRun).length === 2,
);
ok(
  'empty-supply: a gameLost directive carries the reason',
  esRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'empty-supply'),
);
ok(
  'empty-supply: replays are byte-identical (determinism)',
  identical(goldenLowSupply, { seed: 'mvp-runtime-seed', playerCount: 1 }, esStream),
);
// supply exhaustion is structural, not RNG-driven → seed-independent outcome:
ok(
  'empty-supply: outcome is seed-independent (structural, not RNG)',
  lastOf(engine.replay(goldenLowSupply, { seed: 'another-seed', playerCount: 1 }, esStream)).state
    .outcome.reason === 'empty-supply',
);
...
ok(
  'out-of-time: reaches a clean LOSS',
  oot.final.status === 'lost',
  'got ' + oot.final.status + ' / ' + oot.final.state.outcome.reason,
);
...
ok(
  'out-of-time: ample supply never emptied (supply > 0 at loss)',
  oot.final.state.skulls.supply > 0,
);
ok(
  'out-of-time: a gameLost directive carries the reason',
  oot.final.state.outcome.reason === 'out-of-time',
);
// the captured (seed-variable) stream replays byte-identically:
ok(
  'out-of-time: captured stream replays byte-identically (determinism)',
  identical(goldenAmpleSupply, ootOpts, oot.inputs),
);
...
const ootSeeds = ['different-seed', 's2', 's3', 'alpha'].map((s) =>
  autoDrive(goldenAmpleSupply, { seed: s, playerCount: 1 }, passDriver),
);
ok(
  'out-of-time: outcome is seed-independent (always out-of-time)',
  ootSeeds.every((o) => o.final.state.outcome.reason === 'out-of-time'),
);
ok(
  'out-of-time: realized walk length is seed-driven (lengths are not all identical)',
...
ok(
  'multi-hero: init builds two heroes',
  Object.keys(mh.state.heroes).length === 2 && mh.state.heroes.hero1 && mh.state.heroes.hero2,
);
ok(
  'multi-hero: per-player home-kingdom ownership (north→hero1, south→hero2)',
  mh.state.kingdoms.ownership.north === 'hero1' && mh.state.kingdoms.ownership.south === 'hero2',
);
ok(
  'multi-hero: the unowned kingdoms are dormant (east, west)',
  JSON.stringify(mh.state.kingdoms.dormant) === JSON.stringify(['east', 'west']),
);
ok(
  'multi-hero: clockwise seating turnOrder = [hero1, hero2]',
  JSON.stringify(mh.state.clock.turnOrder) === JSON.stringify(['hero1', 'hero2']),
);
ok(
  'multi-hero: month 1 first player is hero1',
  mh.state.clock.activeHero === 'hero1' && mh.state.clock.firstPlayerOfMonth === 'hero1',
);
// a 4-player game has no dormant kingdoms (schema v0.4 byPlayerCount.4 maxItems:0 / canonical fallback)
ok(
  'multi-hero: a 4-player init owns all four kingdoms with none dormant',
  (() => {
    const i4 = engine.init(golden, { seed: 's', playerCount: 4 });
    return (
      i4.state.kingdoms.dormant.length === 0 &&
      Object.keys(i4.state.kingdoms.ownership).length === 4
    );
  })(),
);
...
const tradeStream = [
  act('trade'),
  tr({ from: 'hero1', to: 'hero2', give: [{ asset: 'warriors', amount: 2 }] }),
  obs(0),
];
...
ok(
  'multi-hero trade: total warriors conserved (atomic transfer)',
  tradeF.state.heroes.hero1.warriors + tradeF.state.heroes.hero2.warriors === 14,
);
ok(
  'multi-hero trade: a ui.update records the transfer',
  tradeRun
    .flatMap((r) => r.directives)
    .some(
      (d) =>
        ...
        d.delta &&
        d.delta.trade &&
        d.delta.trade.from === 'hero1' &&
        d.delta.trade.to === 'hero2',
    ),
);
ok(
  "multi-hero trade: the seat rotated to hero2 after hero1's turn",
  tradeF.state.clock.activeHero === 'hero2',
);
ok(
  'multi-hero trade: stream replays byte-identically (determinism)',
  identical(golden, mhOpts, tradeStream),
);

// --- a trade that the giver cannot afford faults (never applied partially) ---
let tradeFaulted = false;
try {
  engine.replay(golden, mhOpts, [
    act('trade'),
    tr({ from: 'hero1', to: 'hero2', give: [{ asset: 'warriors', amount: 999 }] }),
  ]);
} catch (e) {
  tradeFaulted = e.isFault === true;
}
...
ok(
  'multi-hero rotation: month 1 alternates hero1/hero2 clockwise',
  m1.length >= 2 && m1.every((t, i) => t.hero === (i % 2 === 0 ? 'hero1' : 'hero2')),
);
ok(
  "multi-hero rotation: month 2 first player is left of month 1's final player",
  (() => {
...
const xhStream = [
  ...
  dmove('E'),
  dleave(),
  obs(0), // hero1 clears entry+hall, turn ends, rotate
  act('dungeon'),
  dmove('E'),
  dmove('E'), // hero2 re-enters cleared rooms, clears target
];
...
ok(
  'cross-hero dungeon: hero1 cleared ≥1 room → clearedRooms grew (entry+hall)',
  xhHero1Left.length === 2 &&
    xhHero1Left.includes('vault-entry') &&
    xhHero1Left.includes('vault-hall'),
);
ok(
  'cross-hero dungeon: the seat rotated to hero2 before the second entry',
  xhRun[6].state.clock.activeHero === 'hero2',
);
ok(
  "cross-hero dungeon: hero2's entry sees the IDENTICAL clearedRooms set hero1 left (persists per-dungeon)",
...
ok(
  'cross-hero dungeon: cleared rooms are NOT re-resolved for hero2 (spirit 1, warriors 7 untouched)',
...
ok(
  'cross-hero dungeon: hero1 (who resolved the rooms) shows the room effects (spirit 1, warriors 8)',
  xhF.state.heroes.hero1.spirit === 1 && xhF.state.heroes.hero1.warriors === 8,
);
ok(
  'cross-hero dungeon: hero2 finishes the dungeon (3rd room cleared, quest complete)',
  xhF.state.dungeons[DUN].clearedRooms.length === 3 &&
    xhF.state.dungeons[DUN].clearedRooms.includes('vault-target') &&
    xhF.state.quests['azkol-vault'] &&
    xhF.state.quests['azkol-vault'].complete === true,
);
ok(
  'cross-hero dungeon: the stream replays byte-identically (lockstep determinism)',
  identical(golden, mhOpts, xhStream),
);
...
ok(
  'cross-hero dungeon (sanity): the 1P run of the same stream completes the dungeon consistently',
  xh1p.state.dungeons[DUN].clearedRooms.length === 3 &&
    xh1p.state.quests['azkol-vault'].complete === true &&
    xh1p.state.clock.activeHero === 'hero1',
);
ok(
  'cross-hero dungeon (sanity): the 1P run replays byte-identically',
...
const t5 = autoDrive(goldenAmpleSupply, mhOpts, (id, state) => {
  if (id === 'action') {
    t5turns.push({
      month: state.clock.month,
      hero: state.clock.activeHero,
      fpm: state.clock.firstPlayerOfMonth,
    });
    return act('pass');
  }
  return obs(0);
...
ok(
  'multi-hero terminal: a 2P game reaches a clean LOSS',
  t5.final.status === 'lost',
  'got ' + t5.final.status + ' / ' + t5.final.state.outcome.reason,
);
ok(
  'multi-hero terminal: loss reason is out-of-time at the end of month 6',
  t5.final.state.outcome.reason === 'out-of-time' && t5.final.state.clock.month === 6,
);
ok(
  'multi-hero terminal: ample supply never emptied (supply > 0 at the loss)',
  t5.final.state.skulls.supply > 0,
);
...
ok(
  'multi-hero terminal: the game spanned all six months',
  monthsSeen.length === 6 && monthsSeen[0] === 1 && monthsSeen[5] === 6,
);
ok(
  'multi-hero terminal: rotation held — seats strictly alternate within every month',
  monthsSeen.every((m) => {
    const s = seqOf(m);
    return s.length >= 1 && s.every((h, i) => i === 0 || h !== s[i - 1]);
  }),
);
...
ok(
  'multi-hero terminal: the §107 first-player rule held across every month boundary (months 2–6)',
  monthsSeen.slice(1).every((m) => {
    const prev = seqOf(m - 1);
    const fpm = seqOf(m)[0];
    return fpm === next2(prev[prev.length - 1]) && fpm === t5turns.find((t) => t.month === m).fpm;
  }),
);
ok(
  'multi-hero terminal: the captured (seed-variable) stream replays byte-identically',
  identical(goldenAmpleSupply, mhOpts, t5.inputs),
);
...
ok(
  'authored loss: reaches a clean LOSS',
  alF.status === 'lost',
  'got ' + alF.status + ' / ' + alF.state.outcome.reason,
);
ok(
  'authored loss: reason is loss-condition (authored, not an engine-intrinsic default)',
  alF.state.outcome.reason === 'loss-condition',
);
ok(
  'authored loss: a gameLost directive carries the authored reason',
  alRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
ok(
  "authored loss: it fired below the main goal's threshold (goalProgress 2, main goal NOT complete)",
  alF.state.counters.goalProgress === 2 && alF.state.mainGoalComplete === false,
);
ok(
  'authored loss: the guard holds until the threshold (one quest does NOT end the game)',
  lastOf(
    engine.replay(goldenAuthoredLoss, { seed: 'mvp-runtime-seed', playerCount: 1 }, [act('quest')]),
  ).status === 'awaitingInput',
);
ok(
  'authored loss: the stream replays byte-identically (lockstep determinism)',
...
ok(
  'multi-hero win: a 2P game reaches a clean WIN',
  mhWinF.status === 'won',
  'got ' + mhWinF.status + ' / ' + mhWinF.state.outcome.reason,
);
ok(
  'multi-hero win: win reason is adversary-defeated',
  mhWinF.state.outcome.reason === 'adversary-defeated',
);
ok(
  'multi-hero win: main goal completed, adversary spawned then defeated',
  mhWinF.state.mainGoalComplete === true &&
    mhWinF.state.adversary.spawned &&
    mhWinF.state.adversary.defeated,
);
// the acting seat captured at each `action` boundary across the build:
const mhWinActors = mhWin
  .filter((r) => r.awaiting && r.awaiting.id === 'action')
  .map((r) => r.state.clock.activeHero);
ok(
  'multi-hero win: the seat rotated mid-build (4 action turns spanning BOTH seats, not all hero1)',
  mhWinActors.length === 4 && new Set(mhWinActors).size === 2,
);
ok(
  'multi-hero win: a DIFFERENT seat resolved the battle than opened the build (hero1 opened, hero2 battled)',
  mhWinActors[0] === 'hero1' &&
    mhWinActors[3] === 'hero2' &&
    mhWinF.state.clock.activeHero === 'hero2',
);
...
ok(
  'multi-hero win: win path tolerates quest alternation (3 quests from ≥2 seats; global goalProgress reaches 3)',
  new Set(mhWinActors.slice(0, 3)).size >= 2 && mhWinF.state.counters.goalProgress === 3,
);
ok(
  'multi-hero win: the battling seat took no warrior loss (all 5 cards cleared at 5 spend)',
  mhWinF.state.heroes.hero2.warriors === 7,
);
ok(
  'multi-hero win: the scripted stream replays byte-identically (lockstep determinism)',
  identical(golden, mhOpts, winStream),
);
...
const mhWinSeeds = ['different-seed', 's2', 's3', 'alpha'].map((s) =>
  lastOf(engine.replay(golden, { seed: s, playerCount: 2 }, winStream)),
);
ok(
  'multi-hero win: outcome is seed-independent (always won/adversary-defeated, always the rotated seat hero2)',
  mhWinSeeds.every(
    (f) =>
      f.status === 'won' &&
      f.state.outcome.reason === 'adversary-defeated' &&
      f.state.clock.activeHero === 'hero2',
  ),
);
...
ok(
  'authored loss (flag): reaches a clean LOSS',
  flF.status === 'lost',
  'got ' + flF.status + ' / ' + flF.state.outcome.reason,
);
ok(
  'authored loss (flag): reason is loss-condition (authored, on a non-counter subject)',
  flF.state.outcome.reason === 'loss-condition',
);
ok(
  'authored loss (flag): the triggering flag was set by real gameplay (vaultCleared via the dungeon target)',
  flF.state.flags.vaultCleared === true,
);
ok(
  'authored loss (flag): a gameLost directive carries the authored reason',
  flRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
...
ok(
  'authored loss (flag): the guard holds through the dungeon turn (clearing the vault does NOT end the game)',
  flRun[4].state.flags.vaultCleared === true &&
    flRun[4].status === 'awaitingInput' &&
    flRun[4].state.outcome.reason === null,
);
ok(
  'authored loss (flag): the stream replays byte-identically (lockstep determinism)',
  identical(goldenAuthoredLossFlag, alfOpts, flStream),
);
...
ok(
  'warded block: the run completes without terminating and ends the turn',
  wvF.status === 'awaitingInput' && wvF.state.outcome.reason === null,
);
ok(
  'warded block: a dungeonRoomBlocked directive fired for the warded hall',
  wvRun
    .flatMap((r) => r.directives)
    .some(
      (d) => d.type === 'log.entry' && d.event === 'dungeonRoomBlocked' && d.room === 'vault-hall',
    ),
);
ok(
  'warded block: onFail applied — the blocked hero gained a corruption (0 → 1)',
  wvF.state.heroes.hero1.corruption === 1,
);
ok(
  'warded block: the hero was force-left out of the dungeon (no active dungeon subflow remains)',
  !wvF.state.clock.dungeon,
);
ok(
  'warded block: the warded hall is NOT cleared (only the entrance the hero actually resolved)',
  wvCleared.length === 1 && wvCleared.includes('vault-entry') && !wvCleared.includes('vault-hall'),
);
ok(
  'warded block: the spiritCost was NOT debited on the blocked entry (spirit stays 2 from the entrance gain)',
  wvF.state.heroes.hero1.spirit === 2,
);
ok(
  'warded block: the stream replays byte-identically (lockstep determinism)',
  identical(goldenWardedVault, wvOpts, wvStream),
);
...
ok(
  'authored loss (resource): reaches a clean LOSS',
  arF.status === 'lost',
  'got ' + arF.status + ' / ' + arF.state.outcome.reason,
);
ok(
  'authored loss (resource): reason is loss-condition (authored, on a resource subject)',
  arF.state.outcome.reason === 'loss-condition',
);
ok(
  'authored loss (resource): the triggering resource was raised by real gameplay (reinforce: warriors 7 → 9)',
  arF.state.heroes.hero1.warriors === 9,
);
ok(
  'authored loss (resource): a gameLost directive carries the authored reason',
  arRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
...
ok(
  'authored loss (resource): the guard holds below the threshold (a pass turn keeps warriors 7 and does NOT end the game)',
  (() => {
    const h = lastOf(engine.replay(goldenAuthoredLossResource, arOpts, [act('pass')]));
    return (
      h.status === 'awaitingInput' &&
      h.state.heroes.hero1.warriors === 7 &&
      h.state.outcome.reason === null
    );
  })(),
);
ok(
  'authored loss (resource): the stream replays byte-identically (lockstep determinism)',
  identical(goldenAuthoredLossResource, arOpts, arStream),
);
...
ok(
  'authored loss (sealsRemoved): reaches a clean LOSS',
  asF.status === 'lost',
  'got ' + asF.status + ' / ' + asF.state.outcome.reason,
);
ok(
  'authored loss (sealsRemoved): reason is loss-condition (authored, on the sealsRemoved subject)',
  asF.state.outcome.reason === 'loss-condition',
);
ok(
  'authored loss (sealsRemoved): the threshold was reached by real gameplay (two seal.remove breaks → sealsRemoved 2)',
  asF.state.sealsRemoved === 2,
);
ok(
  'authored loss (sealsRemoved): brokenSeals tracks the two engine-defaulted seal ids',
  JSON.stringify(asF.state.brokenSeals) === JSON.stringify(['1-north', '2-north']),
);
ok(
  'authored loss (sealsRemoved): a gameLost directive carries the authored reason',
  asRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
ok(
  'authored loss (sealsRemoved): a sealRemoved event was raised by the break (engine-owned mirror advanced)',
  asRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'sealRemoved'),
);
...
ok(
  'authored loss (sealsRemoved): the guard holds at one seal (turn 1 ends awaitingInput with sealsRemoved 1)',
  asRun[1].state.sealsRemoved === 1 &&
    asRun[1].status === 'awaitingInput' &&
    asRun[1].state.outcome.reason === null,
);
ok(
  'authored loss (sealsRemoved): the stream replays byte-identically (lockstep determinism)',
  identical(goldenAuthoredLossSeal, asOpts, asStream),
);
...
const battleSeatFor = (pc) => {
  const f = lastOf(
    engine.replay(golden, { seed: 'mvp-runtime-seed', playerCount: pc }, winStream3),
  );
  return f;
};
const w3 = battleSeatFor(3);
ok(
  'multi-hero win (3P): reaches a clean WIN (adversary-defeated)',
  w3.status === 'won' && w3.state.outcome.reason === 'adversary-defeated',
  'got ' + w3.status + ' / ' + w3.state.outcome.reason,
);
ok(
  "multi-hero win (3P): the battle resolved on the rotation-determined seat (action 4 wraps to hero1, distinct from 2P's hero2)",
  w3.state.clock.activeHero === 'hero1' &&
    w3.state.adversary.defeated &&
    w3.state.counters.goalProgress === 3,
);
ok(
  'multi-hero win (3P): the scripted stream replays byte-identically (lockstep determinism)',
  identical(golden, { seed: 'mvp-runtime-seed', playerCount: 3 }, winStream3),
);
ok(
  'multi-hero win (3P): outcome is seed-independent (always won/adversary-defeated, always the rotated seat hero1)',
  ['different-seed', 's2', 's3', 'alpha']
    .map((s) => lastOf(engine.replay(golden, { seed: s, playerCount: 3 }, winStream3)))
    .every(
      (f) =>
        f.status === 'won' &&
        f.state.outcome.reason === 'adversary-defeated' &&
        f.state.clock.activeHero === 'hero1',
    ),
);
const w4 = battleSeatFor(4);
ok(
  'multi-hero win (4P): reaches a clean WIN, battle on hero4 (the 4th seat, action 4 = turnOrder[3])',
  w4.status === 'won' &&
    w4.state.outcome.reason === 'adversary-defeated' &&
    w4.state.clock.activeHero === 'hero4' &&
    w4.state.counters.goalProgress === 3,
);
ok(
  'multi-hero win (4P): replays byte-identically and is seed-independent (always won/adversary-defeated, always hero4)',
  identical(golden, { seed: 'mvp-runtime-seed', playerCount: 4 }, winStream3) &&
    ['different-seed', 's2', 's3', 'alpha']
      .map((s) => lastOf(engine.replay(golden, { seed: s, playerCount: 4 }, winStream3)))
      .every(
        (f) =>
          f.status === 'won' &&
          f.state.outcome.reason === 'adversary-defeated' &&
          f.state.clock.activeHero === 'hero4',
      ),
);
...
ok(
  'multi-hero authored loss: reaches a clean LOSS',
  mhalF.status === 'lost',
  'got ' + mhalF.status + ' / ' + mhalF.state.outcome.reason,
);
ok(
  'multi-hero authored loss: reason is loss-condition (authored)',
  mhalF.state.outcome.reason === 'loss-condition',
);
ok(
  'multi-hero authored loss: the loss fired on the ROTATED seat hero2 (not the opener hero1)',
  mhalF.state.clock.activeHero === 'hero2',
);
...
ok(
  'multi-hero authored loss: per-acting-hero — hero2 (who reinforced) is at 9, hero1 (who passed) untouched at 7',
...
ok(
  "multi-hero authored loss: the guard held on hero1's turn (hero1 active, warriors 7, no loss)",
  mhalRun[1].state.clock.activeHero === 'hero1' &&
    mhalRun[1].state.heroes.hero1.warriors === 7 &&
    mhalRun[1].status === 'awaitingInput' &&
    mhalRun[1].state.outcome.reason === null,
);
ok(
  'multi-hero authored loss: a gameLost directive carries the authored reason',
  mhalRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
...
ok(
  'multi-hero authored loss: symmetric — the same guard fires on hero1 when hero1 reinforces first (hero1 at 9, hero2 untouched at 7)',
  mhalSym.status === 'lost' &&
    mhalSym.state.clock.activeHero === 'hero1' &&
    mhalSym.state.heroes.hero1.warriors === 9 &&
    mhalSym.state.heroes.hero2.warriors === 7,
);
ok(
  'multi-hero authored loss: the stream replays byte-identically (lockstep determinism)',
  identical(goldenAuthoredLossResource, mhalOpts, mhalStream),
);
...
ok(
  'foeOnSpace (unit): gte 1 is true with one foe on the space',
  engine.evalCondition(cond('foeOnSpace', 'gte', 1, 'the-tower'), afRun[1].state) === true,
);
ok(
  'foeOnSpace (unit): gte 2 is false with only one foe on the space (guard boundary)',
  engine.evalCondition(cond('foeOnSpace', 'gte', 2, 'the-tower'), afRun[1].state) === false,
);
ok(
  'foeOnSpace (unit): a different space reads 0 (keyed on location, not a global foe count)',
  engine.evalCondition(cond('foeOnSpace', 'gte', 1, 'elsewhere'), afRun[1].state) === false,
);
// lockstep stream:
ok(
  'authored loss (foeOnSpace): reaches a clean LOSS',
  afF.status === 'lost',
  'got ' + afF.status + ' / ' + afF.state.outcome.reason,
);
ok(
  'authored loss (foeOnSpace): reason is loss-condition (authored, on the foeOnSpace subject)',
  afF.state.outcome.reason === 'loss-condition',
);
ok(
  'authored loss (foeOnSpace): the threshold was reached by real gameplay (two foe.spawn → 2 foes on the-tower)',
  afF.state.foes.filter((f) => f.location === 'the-tower').length === 2,
);
ok(
  'authored loss (foeOnSpace): a gameLost directive carries the authored reason',
  afRun
    .flatMap((r) => r.directives)
    .some((d) => d.type === 'log.entry' && d.event === 'gameLost' && d.reason === 'loss-condition'),
);
ok(
  'authored loss (foeOnSpace): the guard holds at one foe (turn 1 ends awaitingInput with one foe on the space)',
  afRun[1].state.foes.filter((f) => f.location === 'the-tower').length === 1 &&
    afRun[1].status === 'awaitingInput' &&
    afRun[1].state.outcome.reason === null,
);
ok(
  'authored loss (foeOnSpace): the stream replays byte-identically (lockstep determinism)',
  identical(goldenAuthoredLossFoe, afOpts, afStream),
);
...
ok(
  'heroAtLocation (unit): gte 1 is true when hero1 is on the space',
  engine.evalCondition(cond('heroAtLocation', 'gte', 1, 'the-village'), _halBase) === true,
);
ok(
  'heroAtLocation (unit): gte 2 is false when only one hero is on the space',
  engine.evalCondition(cond('heroAtLocation', 'gte', 2, 'the-village'), _halBase) === false,
);
ok(
  'heroAtLocation (unit): eq 0 is true for a different space (location-keyed, not global)',
  engine.evalCondition(cond('heroAtLocation', 'eq', 0, 'elsewhere'), _halBase) === true,
);
ok(
  'heroAtLocation (unit): null location does not match any key',
...
const validate = ajv.compile(
  JSON.parse(
    fs.readFileSync(
      require('path').join(__dirname, '../../creator-schema/src/scenario.schema.json'),
      'utf8',
    ),
  ),
);
ok(
  'L1: goldenLowSupply is schema-valid',
  validate(goldenLowSupply),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAmpleSupply is schema-valid',
  validate(goldenAmpleSupply),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAuthoredLoss (winloss.lossCondition clone) is schema-valid',
  validate(goldenAuthoredLoss),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAuthoredLossFlag (flag-subject lossCondition clone) is schema-valid',
  validate(goldenAuthoredLossFlag),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenWardedVault (higher-spiritCost dungeon clone) is schema-valid',
  validate(goldenWardedVault),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAuthoredLossResource (resource-subject lossCondition clone) is schema-valid',
  validate(goldenAuthoredLossResource),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAuthoredLossSeal (sealsRemoved-subject lossCondition clone, effect.apply seal.remove driver) is schema-valid',
  validate(goldenAuthoredLossSeal),
  JSON.stringify(validate.errors),
);
ok(
  'L1: goldenAuthoredLossFoe (foeOnSpace-subject lossCondition clone, effect.apply foe.spawn driver) is schema-valid',
  validate(goldenAuthoredLossFoe),
  JSON.stringify(validate.errors),
);
...
// =====================================================================================
const allDrops = dropTriggers(esRun)
  .concat(dropTriggers(tradeRun))
  .concat(dropTriggers(xhRun))
  .concat(dropTriggers(alRun))
  .concat(dropTriggers(mhWin))
  .concat(dropTriggers(flRun))
  .concat(dropTriggers(wvRun))
  .concat(dropTriggers(arRun))
  .concat(dropTriggers(asRun))
  .concat(dropTriggers(mhalRun))
  .concat(dropTriggers(afRun))
  .concat(oot.final ? dropTriggers(engine.replay(goldenAmpleSupply, ootOpts, oot.inputs)) : []);
ok(
  'skull invariant: no skull.dropTrigger across the corpus carries a count',
