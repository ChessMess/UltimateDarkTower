...
ok(
  'golden fixture reaches a CLEAN WIN',
  winFinal.status === 'won',
  'got ' + winFinal.status + ' / ' + winFinal.state.outcome.reason,
);
...
ok(
  'win: adversary was spawned then defeated',
  winFinal.state.adversary.spawned && winFinal.state.adversary.defeated,
);
ok(
  'win: battle cleared all cards (no warrior loss at 5 spend)',
  winFinal.state.heroes.hero1.warriors === 7,
);
...
ok(
  'golden fixture reaches a CLEAN LOSS',
  lossFinal.status === 'lost',
  'got ' + lossFinal.status + ' / ' + lossFinal.state.outcome.reason,
);
...
function allDirectives(run) {
  return run.flatMap((r) => r.directives);
}
const towerPrograms = allDirectives(winRun)
  .concat(allDirectives(lossRun))
  .filter((d) => d.type === 'tower.program');
const dropTriggers = towerPrograms.filter((d) =>
  (d.ops || []).some((o) => o.channel === 'skull.dropTrigger'),
);
ok(
  'skull.dropTrigger directives carry no count (skull invariant)',
...
ok(
  'win outcome is seed-independent (authored logic, not RNG)',
  winRun2[winRun2.length - 1].status === 'won',
);
...
winRun.forEach((r, i) =>
  console.log(
    `  step ${i}: ${r.status}` +
      (r.awaiting ? ` awaiting ${r.awaiting.id}` : '') +
