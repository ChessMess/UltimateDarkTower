...
(() => {
  ...
  ok(
    '1P: init pauses at the heroSelect boundary',
    r0.status === 'awaitingInput' && r0.awaiting && r0.awaiting.id === 'heroSelect',
  );
  ok(
    '1P: the request offers the full authored pool',
    r0.awaiting &&
      JSON.stringify(r0.awaiting.options) ===
        JSON.stringify([{ id: 'brutal-warlord' }, { id: 'orphaned-scion' }]),
  );
  ...
  ok(
    '1P: after the pick, hero1.heroRef is assigned',
    r1.state.heroes.hero1.heroRef === 'orphaned-scion',
  );
  ok(
    '1P: the selection cursor is cleared once all seats are done',
    r1.state.clock.heroSelect === null,
  );
  ok(
    '1P: the run proceeds past the node (no longer awaiting heroSelect)',
    !(r1.awaiting && r1.awaiting.id === 'heroSelect'),
  );
  const logged = r1.directives.find((d) => d.type === 'log.entry' && d.event === 'heroSelected');
  ok(
    '1P: a heroSelected log entry is emitted',
    logged && logged.hero === 'hero1' && logged.heroId === 'orphaned-scion',
  );
})();
...
(() => {
  ...
  for (let i = 0; i < 4; i++) {
    ok(
      '4P: seat ' + (i + 1) + ' is awaiting heroSelect',
      r.status === 'awaitingInput' && r.awaiting.id === 'heroSelect',
    );
    ...
    ok(
      '4P: seat ' + (i + 1) + ' pool excludes already-claimed heroes',
      claimedSoFar.every((h) => !offered.includes(h)),
    );
    r = engine.step(r.state, heroSel(picks[i]));
  }
  ok(
    '4P: seats are asked in turnOrder index order 0,1,2,3',
    JSON.stringify(askedSeats) === JSON.stringify([0, 1, 2, 3]),
  );
  const refs = ['hero1', 'hero2', 'hero3', 'hero4'].map((h) => r.state.heroes[h].heroRef);
  ok(
    '4P: every seat received the hero it picked, in order',
    JSON.stringify(refs) === JSON.stringify(picks),
  );
