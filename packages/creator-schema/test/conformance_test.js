...
try {
  validate = ajv.compile(
    JSON.parse(
      fs.readFileSync(require('path').join(__dirname, '../src/scenario.schema.json'), 'utf8'),
    ),
  );
...
function check(name, instance, expectValid) {
  ...
  console.log(
    `${correct ? 'PASS' : 'XXXX'}  ${name}  (expected ${expectValid ? 'valid' : 'INVALID'}, got ${ok ? 'valid' : 'invalid'})`,
  );
  if (!correct) {
    fail++;
    if (validate.errors)
...
let t = clone(base);
t.library.buildingTypes.citadel.free[0] = { op: 'teleport.hero' };
check('unknown effect op rejected', t, false);

// Negative: extra prop on a closed effect
t = clone(base);
t.library.buildingTypes.village.free[0] = {
  op: 'resource.gain',
  resource: 'warriors',
  amount: 2,
  bogus: 1,
...
t = clone(base);
t.graph.nodes[3].props.towerOp = { channel: 'skull.dropTrigger', count: 3 };
...
t = clone(base);
t.meta.provenance.importedSeed.seed = 'not-a-seed';
